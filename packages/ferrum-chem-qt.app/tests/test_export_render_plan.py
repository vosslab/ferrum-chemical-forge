...
def _content_scene(qapp: object) -> object:
	...
	scene = ferrum_qt.canvas.scene.ChemScene()
...
#============================================
def _dispose_content_callbacks(scene: ferrum_qt.canvas.scene.ChemScene) -> None:
	...
	for item in list(scene.items()):
		if item is paper or item is grid_overlay:
			continue
		ferrum_qt.canvas.document_projection.dispose_item_callbacks(item)
#============================================
def test_svg_crop_plan_uses_content_without_paper_or_grid(qapp: object) -> None:
	...
	with _content_scene(qapp) as scene:
		...
		plan = ferrum_qt.io.render_plan.build_render_plan(scene, "svg")
		assert (
...
def test_png_and_pdf_plans_keep_the_paper_page(qapp: object) -> None:
	...
	with _content_scene(qapp) as scene:
		scene._paper_attributes = {"crop_svg": "1", "crop_margin": "7"}
		png_plan = ferrum_qt.io.render_plan.build_render_plan(scene, "png")
		pdf_plan = ferrum_qt.io.render_plan.build_render_plan(scene, "pdf")
		assert (
...
def test_svg_export_writes_an_svg_artifact(qapp: object, tmp_path: object) -> None:
	...
	with _content_scene(qapp) as scene:
		ferrum_qt.io.export.export_svg(scene, str(path))
...
def test_png_export_writes_a_png_artifact(qapp: object, tmp_path: object) -> None:
	...
	with _content_scene(qapp) as scene:
		ferrum_qt.io.export.export_png(scene, str(path))
...
def test_pdf_export_writes_a_pdf_artifact(qapp: object, tmp_path: object) -> None:
	...
	with _content_scene(qapp) as scene:
		ferrum_qt.io.export.export_pdf(scene, str(path))
...
def test_selection_projection_carries_an_atom_attached_mark(qapp: object) -> None:
	...
	with _content_scene(qapp) as scene:
		...
		ferrum_qt.canvas.document_projection.create_mark_item(mark_model, atom_item)
		atom_item.setSelected(True)
		projection = ferrum_qt.io.render_plan.project_supported_items(
...
def test_export_projection_disposal_retires_its_temporary_scene(
	...
	with _content_scene(qapp) as scene:
		projection = ferrum_qt.io.render_plan.project_supported_items(scene)
...
def test_export_projection_uses_one_child_first_terminal_owner(
	...
	with _content_scene(qapp) as scene:
		...
		monkeypatch.setattr(
			ferrum_qt.io.render_plan.PySide6.QtWidgets,
...
def test_failed_export_projection_retires_earlier_atom_callbacks(
	...
	with _content_scene(qapp) as scene:
		first_molecule = ferrum_qt.models.molecule_model.MoleculeModel()
		first_atom = first_molecule.create_atom("C")
		first_molecule.add_atom(first_atom)
		second_molecule = ferrum_qt.models.molecule_model.MoleculeModel()
		...
		for molecule, atom in ((first_molecule, first_atom), (second_molecule, second_atom)):
			source_item = ferrum_qt.canvas.items.atom_item.AtomItem(atom)
			source_item.molecule_model = molecule
			scene.addItem(source_item)
			source_items.append(source_item)
		callback_calls = []
		created_items = []
		original_atom_item = ferrum_qt.canvas.items.atom_item.AtomItem
		...
		monkeypatch.setattr(
			ferrum_qt.canvas.items.atom_item, "AtomItem", construct_or_fail,
		)
		monkeypatch.setattr(
			ferrum_qt.io.render_plan.PySide6.QtWidgets,
		...
		with pytest.raises(RuntimeError, match="temporary scene adoption failed") as failure:
			ferrum_qt.io.render_plan.project_supported_items(scene, source_items)
		created_items[0].atom_model.symbol = "N"
		assert (
			callback_calls == []
			and ferrum_qt.canvas.graphics_retirement.temporary_scene_retirement_reaper

"""Ferrum background preparation for existing-molecule coordinates."""
...
#============================================
class FerrumNativeCoordinatePreparationWorker(FerrumDetachedJobThread):
	...
	def __init__(self, observation: object, molecule_id: str) -> None:
		...
		self._prepare_operation = engine.prepare_molecule_coordinates_v1
		self._prepare_arguments = (observation, molecule_id)
		self.success_message = "Generated Ferrum coordinates."
		super().__init__(
			lambda: self._prepare_operation(*self._prepare_arguments),
			lambda error: FerrumNativeCoordinatePreparationFailure(
	...
	def _emit_success(self, prepared: object) -> None:
		"""Retain the coordinate route's established prepared signal."""
...
class FerrumNativeCleanGeometryPreparationWorker(
	...
	def __init__(
		...
		"""Capture exact immutable targets before Ferrum generation begins."""
		if type(observation) is not engine.SessionDocumentObservationV1:
		...
		self._prepare_operation = engine.prepare_clean_geometry_v1
		self._prepare_arguments = (
			observation, molecule_ids, target_spacing_points,
		)
		self.success_message = "Cleaned Ferrum molecule geometry."
		FerrumDetachedJobThread.__init__(
			self, lambda: self._prepare_operation(*self._prepare_arguments),
			lambda error: FerrumNativeCoordinatePreparationFailure(
				type(error).__name__, str(error),
			),
		)
...
class FerrumNativeCoordinateGenerationWindowMixin:
	...
	def _on_generate_coordinates(self) -> None:
		"""Choose one durable molecule and start Ferrum coordinate generation."""
		...
		if not choices:
			self._show_edit_refusal(self._unavailable_edit_refusal("This document has no durable molecule that Rust can regenerate."))
			return
		choice = choices[0]
		if len(choices) > 1:
		...
		self.statusBar().showMessage(self.tr("Generating Ferrum coordinates..."), 0)
	...
	def _cancel_coordinate_generation(self) -> None:
		"""Invalidate coordinate delivery while worker cleanup finishes normally."""
		intent = self._coordinate_generation_intent
		if intent is None or intent.worker.delivery_cancelled:
			return
		intent.worker.cancel_delivery()
		self.statusBar().showMessage(
			self.tr("Cancelling coordinate delivery; waiting for the current operation to finish..."), 0,
		)
		self._refresh_actions()

	#============================================
	def _coordinate_generation_blocks_tab_close(self, tab: object) -> bool:
		"""Keep the worker's source tab alive until worker cleanup finishes."""
		intent = self._coordinate_generation_intent
		if intent is None or intent.tab is not tab:
			return False
		self._show_edit_refusal(self._unavailable_edit_refusal("Cancel coordinate generation and wait for the current operation before closing."))
		return True

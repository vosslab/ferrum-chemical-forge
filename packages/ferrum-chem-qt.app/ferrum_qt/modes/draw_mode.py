...
_BOND_TYPE_BY_SUBMODE = ferrum_qt.bond_presentation.DRAW_BOND_TYPE_BY_SUBMODE
...
#============================================
class DrawMode(ferrum_qt.modes.base_mode.BaseMode):
	...
	def _restore_result_selection(self, outcome: object) -> None:
		...
		ferrum_qt.canvas.document_projection.select_projected_persistent_keys(scene, keys)
	...
	def _create_atom_at(self, x: float, y: float, symbol: str | None = None) -> ferrum_qt.canvas.items.atom_item.AtomItem | None:
		...
		if undo_stack is not None:
			cmd = ferrum_qt.undo.commands.AddAtomCommand(
	...
	#============================================
	def _create_bond_between(self, atom1_item: ferrum_qt.canvas.items.atom_item.AtomItem, atom2_item: ferrum_qt.canvas.items.atom_item.AtomItem) -> ferrum_qt.canvas.items.bond_item.BondItem | None:
		...
		undo_stack = self._env.undo_stack
		if undo_stack is not None:
			cmd = ferrum_qt.undo.commands.AddBondCommand(
	...
	def _toggle_bond_type(self, bond_item: ferrum_qt.canvas.items.bond_item.BondItem) -> None:
		...
		for prop_name, old_val in [
			...
			if new_val != old_val:
				...
				cmd = ferrum_qt.undo.commands.ChangePropertyCommand(
		...
		if model.atom1 is not old_atom1 or model.atom2 is not old_atom2:
			...
			undo_stack.push(cmd)
			cmd2 = ferrum_qt.undo.commands.ChangePropertyCommand(
	...
	#============================================
	def _merge_atoms(self, survivor_item: ferrum_qt.canvas.items.atom_item.AtomItem, duplicate_item: ferrum_qt.canvas.items.atom_item.AtomItem) -> None:

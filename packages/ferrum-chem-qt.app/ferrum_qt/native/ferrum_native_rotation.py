...
class FerrumNativeRotationSelection:
	"""Immutable projection facts captured for one native rotation gesture."""
...
class FerrumNativeRotationTabMixin:
	...
	def apply_selected_atom_rotation(self, selection: FerrumNativeRotationSelection,
		...
		if type(selection) is not FerrumNativeRotationSelection:
			raise TypeError("native rotation requires exact captured selection facts")
		if (
			...
			raise TypeError("native rotation requires finite float center and angle values")
		current = self.selected_atom_rotation()
		if current.addresses != selection.addresses:
			raise ValueError("native rotation selection changed during the gesture")
		import ferrum_chem
		targets = tuple(
			ferrum_chem.DocumentAtomRotationTargetV1.create(molecule_id, atom_id)

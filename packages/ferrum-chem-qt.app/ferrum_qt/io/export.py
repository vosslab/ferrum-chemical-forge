...
def render_snapshot_capture(capture: object, format_name: str) -> SnapshotExportOutcome:
	...
	result = ferrum_qt.io.snapshot_render.render_request(capture)
...
def _stage_artifact(
	...
	try:
		file_descriptor, staged_path = tempfile.mkstemp(
			prefix=".ferrum-export-", dir=directory,
		)
...
def _export_source(scene: PySide6.QtWidgets.QGraphicsScene,
		format_name: str, margin: int) -> tuple[PySide6.QtWidgets.QGraphicsScene,
			ferrum_qt.io.render_plan.RenderPlan,
			ferrum_qt.io.render_plan.ExportProjection | None]:
	"""Return the non-decorative source scene needed by one render plan."""
	plan = ferrum_qt.io.render_plan.build_render_plan(scene, format_name, margin)
	...
	try:
		...
		plan = ferrum_qt.io.render_plan.build_render_plan(
...
def export_svg(scene: PySide6.QtWidgets.QGraphicsScene, file_path: str,
	...
	with ferrum_qt.io.render_plan.ExportRenderScope(projection, painter):
		# set up the SVG generator
		generator = generator_class()
		generator.setFileName(file_path)
		generator.setSize(PySide6.QtCore.QSize(int(rect.width()), int(rect.height())))
		generator.setViewBox(rect)
		generator.setTitle("Ferrum-Qt Export")
		generator.setDescription("Chemistry structure exported from Ferrum-Qt")

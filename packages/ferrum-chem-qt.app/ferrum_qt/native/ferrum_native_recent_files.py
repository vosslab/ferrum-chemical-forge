"""Personal native Recent Files state and File-menu projection."""
...
class FerrumNativeRecentFiles:
	...
	def __init__(
		...
		if type(preferences) is not ferrum_qt.config.preferences.Preferences:
			raise TypeError("native recent files require the shared Preferences owner")
		if type(capacity) is not int or capacity < 1:
			raise ValueError("native recent-file capacity must be a positive integer")
		self._window = window
	...
	def install_file_menu(
		...
		"""Place the personal submenu between Open and the native save actions."""
	...
	def record_confirmed_path(self, path: str | pathlib.Path) -> None:
		"""Promote one path only after native installation or publication succeeded."""
	...
	def _normalize_display_path(self, path: str | pathlib.Path) -> str:
		...
		if type(path) is not str or not path:
			raise TypeError("native recent paths require a nonempty path string")

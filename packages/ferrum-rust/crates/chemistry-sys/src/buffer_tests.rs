...
use crate::buffer::{FerrumChemOwnedBuffer, OutputBuffer, checked_length, finish_call};
use crate::{AdapterError, FERRUM_CHEM_MAX_RESPONSE_BYTES};
...
fn rejects_an_oversized_foreign_length_before_forming_a_slice() {
    let error = checked_length(FERRUM_CHEM_MAX_RESPONSE_BYTES + 1)

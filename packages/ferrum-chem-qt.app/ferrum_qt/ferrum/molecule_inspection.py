...
#============================================
class FerrumNativeMoleculeInspectionWorker(FerrumDetachedJobThread):
	...
	def __init__(self, observation: object, revision: int, digest: str,
		...
		if type(observation) is not engine.SessionDocumentObservationV1:
			raise TypeError("Ferrum inspection requires an exact Ferrum observation")
		self._arguments = (
			observation, revision, digest,
			tuple(address.molecule_id for address in addresses),
		)
		super().__init__(
			lambda: engine.inspect_document_molecule_information_v1(*self._arguments),
			lambda error: FerrumNativeMoleculeInspectionFailure(str(error)),
		)

	#============================================
	def _emit_success(self, result: object) -> None:
		"""Retain the inspection route's established result signal."""
...
def format_molecule_information(result: object) -> str:
	"""Format retained source facts followed by Ferrum perceived composition."""
	lines = []
...
#============================================
class FerrumNativeMoleculeInformationDialog(FerrumAccessibleDialog):
	"""Resizable, selectable, Close-only Ferrum molecule information."""
...
class FerrumNativeMoleculeInspectionMixin(
	...
	def _build_molecule_inspection_actions(self, menu: PySide6.QtWidgets.QMenu) -> None:
		"""Add the one Ferrum molecule-information action and its cancellation."""
		menu.addSeparator()
	...
	def _on_document_molecule_inspected(self, worker: object, result: object) -> None:
		...
		if intent is None:
			return
		if (
			type(result) is not engine.DocumentMoleculeInformationV1
	...
	def _show_molecule_information_dialog(self, result: object) -> None:
		"""Run one modal read-only dialog owned by the ordinary Ferrum window."""
	...
	def _on_document_molecule_inspection_failed(self, worker: object, failure: object) -> None:
		"""Show a current noncancelled Ferrum receipt failure without fallback."""
	...
	def _molecule_inspection_blocks_tab_close(self, tab: object) -> bool:
		...
		if intent is None or intent.tab is not tab:
			return False
		self._show_edit_refusal(self._unavailable_edit_refusal("Cancel the inspection and wait for the current operation before closing."))
		return True

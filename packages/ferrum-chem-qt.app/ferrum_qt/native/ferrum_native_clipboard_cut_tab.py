"""Authenticated Cut commit client for one Rust-native document tab."""
...
class FerrumNativeClipboardCutTabMixin:
	...
	def apply_prepared_clipboard_cut(self, prepared: object,
		...
		import ferrum_chem
		if type(prepared) is not ferrum_chem.DocumentClipboardCutPlanV1:
			raise TypeError("native Cut requires an exact frozen Ferrum plan")
		if type(expected_revision) is not int or type(expected_digest) is not str:
			raise TypeError("native Cut requires exact revision and digest facts")

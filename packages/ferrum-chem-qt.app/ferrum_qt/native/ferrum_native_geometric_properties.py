...
def dialog_model_from_projection(root: object) -> FerrumNativeGeometricDialogModel:
	...
	import ferrum_chem
	if type(root) is not ferrum_chem.PresentationRootProjectionV1:
		raise TypeError("native geometric properties require an exact Ferrum root")
	...
	if root.kind in _BOX_KINDS:
		payload = root.shape
		expected_type = ferrum_chem.BoxShapeProjectionV1
	elif root.kind == "polygon":
		payload = root.polygon
		expected_type = ferrum_chem.PolygonProjectionV1
	else:
		payload = root.polyline
		expected_type = ferrum_chem.PolylineProjectionV1
	...
	if type(payload.target.id) is not str or type(payload.target.source_id) is not str:
		raise ValueError("native geometric properties require a durable authored target")
...
def property_changes_from_dialog(
	...
	if type(changes) is not tuple:
		raise TypeError("native geometric property changes must be an exact tuple")
	import ferrum_chem
	converted: list[object] = []
	for change in changes:
		if type(change) is not tuple or len(change) != 2:
			raise TypeError(
				"native geometric property changes must be exact field/value pairs",
			)
		field, value = change
		if field == "line_width" and type(value) is float:
			converted.append(
				ferrum_chem.DocumentGeometricPropertyChangeV1.line_width(value),
			)
		elif field == "line_color" and type(value) is str:
			converted.append(
				ferrum_chem.DocumentGeometricPropertyChangeV1.stroke_color(value),
			)
		elif field == "area_color" and model.fillable and (
				value is None or type(value) is str):
			converted.append(
				ferrum_chem.DocumentGeometricPropertyChangeV1.fill_color(value),
			)
		else:
			raise ValueError("native geometric form supplied an unsupported property change")
...
def bracket_dialog_model_from_projection(
	...
	import ferrum_chem
	if type(pair) is not ferrum_chem.BracketPairProjectionV1:
		raise TypeError("native bracket properties require an exact Ferrum pair")
	if (
	...
	for root in roots:
		if type(root) is not ferrum_chem.PresentationRootProjectionV1:
			raise TypeError("native bracket properties require exact Ferrum roots")
...
def bracket_property_changes_from_dialog(
	...
	if type(changes) is not tuple:
		raise TypeError("native bracket property changes must be an exact tuple")
	import ferrum_chem
	converted: list[object] = []
	for change in changes:
		if type(change) is not tuple or len(change) != 2:
			raise TypeError("native bracket changes must be exact field/value pairs")
		field, value = change
		if field == "line_width" and type(value) is float:
			converted.append(
				ferrum_chem.DocumentBracketPropertyChangeV1.line_width(value),
			)
		elif field == "line_color" and type(value) is str:
			converted.append(
				ferrum_chem.DocumentBracketPropertyChangeV1.line_color(value),
			)
		else:
			raise ValueError("native bracket form supplied an unsupported property change")
	return tuple(converted)


#============================================
class FerrumNativeGeometricPropertiesMixin:
	"""Durable selection and mutation methods mixed into the native document tab."""
	...
	def apply_selected_geometric_properties(self, expected_target_id: str,
		...
		if type(expected_target_id) is not str or type(expected_source_id) is not str:
			raise TypeError("native geometric properties require durable string identifiers")
		if type(changes) is not tuple:
			raise TypeError("native geometric properties require an exact change tuple")
		root = self.selected_geometric_projection()
		model = dialog_model_from_projection(root)
		if (
		...
		import ferrum_chem
		if any(type(change) is not ferrum_chem.DocumentGeometricPropertyChangeV1
				for change in changes):
			raise TypeError("native geometric properties require exact frozen Ferrum changes")
	...
	def apply_selected_bracket_properties(self, expected_pair_id: str,
		...
		if type(expected_pair_id) is not str or type(expected_member_target_ids) is not tuple:
			raise TypeError("native bracket properties require exact durable identifiers")
		if type(changes) is not tuple:
			raise TypeError("native bracket properties require an exact change tuple")
		...
		import ferrum_chem
		if any(type(change) is not ferrum_chem.DocumentBracketPropertyChangeV1
				for change in changes):
			raise TypeError("native bracket properties require exact frozen Ferrum changes")
...
def install_geometric_properties_action(window: object,
	...
	"""Install one native vector-properties action on the public window."""
...
def _on_edit_geometric_properties(window: object) -> None:
	...
	try:
		...
		window._show_native_file_warning("Native Vector Properties Unavailable", str(exc))
		return
	...
	try:
		...
		window._refresh_actions()
		window._show_native_file_warning("Native Vector Properties Error", str(exc))
		return
	window.statusBar().showMessage(window.tr("Updated Rust-native vector appearance."), 5000)
	window._refresh_actions()

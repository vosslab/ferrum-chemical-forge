"""Exact Rust-owned molecule-name editing for the ordinary native window."""
...
# local repo modules
import ferrum_qt.native.ferrum_native_molecule_inspection as native_molecule_inspection
...
class FerrumNativeMoleculeNameTabMixin:
	...
	def set_durable_molecule_name_v1(self, expected_revision: int,
		...
		if (
			...
			raise TypeError("native molecule name requires exact frozen Ferrum inputs")
		if any(
			...
			raise TypeError("native molecule name requires exact durable selection targets")
...
class FerrumNativeMoleculeNameWindowMixin:
	...
	def _build_molecule_name_action(self, menu: PySide6.QtWidgets.QMenu) -> None:
		"""Add the native name action beside the existing molecule information route."""
		menu.addSeparator()
	...
	def _on_set_molecule_name(self) -> bool:
		...
		if not self._molecule_name_capture_is_current(capture):
			self._show_native_file_warning(
				"Set Molecule Name",
				"The active molecule selection changed while the name dialog was open.",
			)
			return False
		try:
			...
			self._show_native_file_warning("Set Molecule Name", str(exc))
			self._refresh_actions()
			return False
		self.statusBar().showMessage(self.tr("Updated the Rust-owned molecule name."), 5000)
		self._refresh_actions()
		return True

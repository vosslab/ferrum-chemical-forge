"""Rust-owned  direct-bond pointer probes for one Ferrum document tab."""
...
class FerrumNativeDirectBondGestureTabMixin:
	...
	def direct_bond_pointer_probe_at_viewport_point(
		...
		if len(atom_ids) == 1:
			hit_state = engine.DirectBondPointerHitState.unique_atom
			direct_atom_id = next(iter(atom_ids))
		elif atom_ids:
			hit_state = engine.DirectBondPointerHitState.ambiguous_atom
			direct_atom_id = None
		else:
			hit_state = engine.DirectBondPointerHitState.none
			direct_atom_id = None
		...
		frame = engine.DirectBondViewportToScene(
		...
		return engine.DirectBondPointerProbe(
	...
	def begin_direct_bond_gesture(
		...
		"""Begin one revision-fenced Rust direct-bond gesture from  pointer facts."""
		...
		return self._session.begin_direct_bond_gesture(
	...
	def resolve_direct_bond_end(self, gesture: object, end_probe: object) -> object:
		"""Resolve one  endpoint into an opaque generic transition request."""
		self._require_mutable()
		#  resolution is a read-only native capability call.  Its explicit
		# session argument must remain the exact PyO3 DocumentSession; the tab's
		# public `_session` is the mutation lifecycle facade used for bound calls.
		return gesture.resolve_end(self._live_document_session_v1, end_probe)

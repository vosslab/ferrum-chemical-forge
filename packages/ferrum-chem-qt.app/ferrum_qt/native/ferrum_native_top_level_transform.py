"""Native actions for Rust-owned complete-root transforms."""
...
class FerrumNativeTopLevelTransformTabMixin:
	...
	def selected_top_level_transform_targets(
		...
		for molecule in self._document_observation.projection.molecules:
			...
			selectors.append(
				ferrum_chem.DocumentTopLevelRootSelectorV1.create(
	...
	def align_selected_top_level_roots(self, alignment: object) -> object:
		...
		self._require_mutable()
		import ferrum_chem
		if type(alignment) is not ferrum_chem.DocumentTopLevelAlignmentV1:
			raise TypeError("native root alignment requires an exact Ferrum value")
		targets, restore = self.selected_top_level_transform_targets()
		operation = ferrum_chem.DocumentOperationV1.align_top_level_roots(
			targets, alignment,
		)
	...
	def translate_selected_top_level_roots(self, dx: float, dy: float) -> object:
		...
		if type(dx) not in (int, float) or type(dy) not in (int, float):
			raise TypeError("native root translation requires exact numeric point deltas")
	...
	def scale_top_level_roots_at_revision(
		...
		self._require_mutable()
		if type(expected_revision) is not int:
			raise TypeError("native root scale requires an exact revision")
		import ferrum_chem
		operation = ferrum_chem.DocumentOperationV1.scale_top_level_roots(
	...
	def mirror_selected_top_level_roots(self, orientation: object) -> object:
		...
		self._require_mutable()
		import ferrum_chem
		if type(orientation) is not ferrum_chem.DocumentTopLevelMirrorV1:
			raise TypeError("native root mirror requires an exact Ferrum value")
		targets, restore = self.selected_top_level_transform_targets()
		operation = ferrum_chem.DocumentOperationV1.mirror_top_level_roots(
			targets, orientation,
		)
		result = self._session.submit(self.current_snapshot.revision, operation)
		self._install_mutation_result(result, restore)
...
class FerrumNativeTopLevelTransformMixin:
	...
	def _on_align_top_level_roots(self, name: str, _checked: bool = False) -> None:
		...
		try:
			import ferrum_chem
			alignment = getattr(ferrum_chem.DocumentTopLevelAlignmentV1, name)
			tab.align_selected_top_level_roots(alignment)
		except Exception as error:
			self._show_native_file_warning("Alignment Failed", str(error))
		finally:
	...
	def _on_scale_top_level_roots(self, _checked: bool = False) -> None:
		...
		try:
			...
			if self._active_native_tab() is not tab:
				raise _tab_error("active native document changed while scale was open")
			tab.scale_top_level_roots_at_revision(
				expected_revision, targets, restore, factors[0], factors[1],
			)
		except Exception as error:
			self._show_native_file_warning("Scale Failed", str(error))
		finally:
			self._refresh_actions()
	...
	def _on_mirror_top_level_roots(
		...
		try:
			import ferrum_chem
			orientation = getattr(ferrum_chem.DocumentTopLevelMirrorV1, name)
			tab.mirror_selected_top_level_roots(orientation)
		except Exception as error:
			self._show_native_file_warning("Mirror Failed", str(error))
		finally:
			self._refresh_actions()
...
def _tab_error(message: str) -> RuntimeError:
	"""Create the native tab's public error without introducing an import cycle."""

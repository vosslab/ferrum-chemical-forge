"""Disposable Qt projection of Rust-issued direct-bond render operations."""
...
#============================================
def create_overlay(tab: object, overlay: object) -> PySide6.QtWidgets.QGraphicsItem:
	"""Paint the complete immutable operation plan issued by Rust admission."""
	operations = overlay.render_operations
	if type(operations) is not tuple:
		raise ValueError("Ferrum direct-bond admission omitted its render operations")
	return create_issued_operations_overlay(tab, operations)


#============================================
def create_issued_operations_overlay(tab: object,
		operations: tuple[object, ...]) -> PySide6.QtWidgets.QGraphicsItemGroup:
	"""Paint immutable Rust-issued directed-bond operations without local geometry."""
	...
	for operation in operations:
		if operation.kind == "path":
			...
			raise ValueError("Rust direct-bond preview returned an unsupported operation")
...
def _path_item(operation: object) -> PySide6.QtWidgets.QGraphicsPathItem:
	"""Copy one issued path without selecting geometry or paint locally."""
	...
	for command in operation.commands:
		if command.kind == "move_to":
			...
			raise ValueError("Rust direct-bond preview path has an unknown command")
...
def _line_item(operation: object) -> PySide6.QtWidgets.QGraphicsLineItem:
	"""Copy one received finite Rust-owned line operation."""

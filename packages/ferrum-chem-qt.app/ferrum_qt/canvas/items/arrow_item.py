...
class ArrowItem(PySide6.QtWidgets.QGraphicsItem):
	...
	def paint(self, painter: PySide6.QtGui.QPainter,
		...
		if self.isSelected() or self._hovered:
			if self.isSelected():
				highlight_color = PySide6.QtGui.QColor(
					ferrum_qt.canvas.items.render_ops_painter.get_canvas_color(
						"selection",
					),
				)
			else:
				highlight_color = PySide6.QtGui.QColor(
					ferrum_qt.canvas.items.render_ops_painter.get_canvas_color("hover"),
				)
		...
		if self._color is None:
			resolved_color = ferrum_qt.canvas.items.render_ops_painter._default_color
		else:
	...
	def _draw_arrowhead(self, painter: PySide6.QtGui.QPainter,
		...
		if self._color is None:
			fill_color = ferrum_qt.canvas.items.render_ops_painter._default_color
		else:

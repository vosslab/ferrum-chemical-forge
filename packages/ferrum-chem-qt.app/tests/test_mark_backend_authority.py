...
#============================================
def _new_session(
		main_window: ferrum_qt.main_window.MainWindow, cdml_text: str = _MARK_CDML,
		) -> object:
...
def _dispose_session(session: object) -> None:
	...
	if not callable(dispose_later):
		raise TypeError("Mark test session has no session-lifecycle owner")
	dispose_later(session)


#============================================
def _mark_mode(session: object) -> ferrum_qt.modes.mark_mode.MarkMode:
	"""Activate and return this session's MarkMode."""
	session.mode_manager.set_mode("mark")
	mode = session.mode_manager.current_mode
	if not isinstance(mode, ferrum_qt.modes.mark_mode.MarkMode):
...
def _mark_request(session: object, revision: int, action: str = "add") -> object:
	...
	return ferrum_qt.models.document_session.build_atom_mark_request(
...
def test_mark_yaml_public_vocabulary_submits_immutable_backend_requests(
	...
	modes_config = yaml.safe_load(
		ferrum_qt.setup.mode_setup.get_modes_yaml_path().read_text(encoding="utf-8"),
	)
...
def test_mark_mode_add_remove_uses_backend_history_and_chemistry_scalars(
	...
	try:
		...
		assert (
			...
			and all(
				not isinstance(item, ferrum_qt.canvas.items.mark_item.MarkItem)
...
def test_edit_delete_selected_mark_uses_its_ordinal_and_backend_history(
	...
	try:
		...
		atom_selected = any(
			isinstance(selected, ferrum_qt.canvas.items.atom_item.AtomItem)

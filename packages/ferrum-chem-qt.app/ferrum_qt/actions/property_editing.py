...
def _bond_properties_submission(
	...
	if hasattr(view, "scene"):
		from ferrum_qt.canvas.scene_queries import find_molecule_for_bond
...
def _atom_properties_submission(
	...
	molecule = None
	if hasattr(view, "scene"):
		from ferrum_qt.canvas.scene_queries import find_molecule_for_atom
...
def capture_selected_plus_properties(
	...
	dialog = ferrum_qt.dialogs.plus_dialog.PlusDialog(font_size, color, parent)
...
def capture_selected_wavy_properties(
	...
	dialog = ferrum_qt.dialogs.wavy_dialog.WavyDialog(width, line_color, parent)
...
def _plain_text_values(text_model: object) -> dict[str, object] | None:
	...
	if fragment is None:
		...
		plain_text = ferrum_qt.io.cdml_inspection.direct_ftext_text(fragment)
...
def edit_atom_properties(
	...
	if synchronized is not None:
		dialog = ferrum_qt.dialogs.atom_dialog.AtomDialog(atom_model, parent)
		if dialog.exec() != dialog.DialogCode.Accepted:
			return False
		changes = dialog.changes()
		if not changes:
	...
	accepted = ferrum_qt.dialogs.atom_dialog.AtomDialog.edit_atom(
		atom_model, parent,
	)
	if not accepted:
		return False
...
def edit_bond_properties(
	...
	if synchronized is not None:
		dialog = ferrum_qt.dialogs.bond_dialog.BondDialog(bond_model, parent)
		if dialog.exec() != dialog.DialogCode.Accepted:
			return False
		changes = dialog.changes()
		if not changes:
	...
	accepted = ferrum_qt.dialogs.bond_dialog.BondDialog.edit_bond(
		bond_model, parent,
	)
	if not accepted:
		return False

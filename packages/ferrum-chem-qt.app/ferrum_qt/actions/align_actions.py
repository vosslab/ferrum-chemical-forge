"""Align menu action registrations for Ferrum-Qt."""
...
def _push_top_level_transform(
	...
	for object_model, dx, dy in objects_and_offsets:
		if isinstance(
			...
			after_bounds = ferrum_qt.geometry.translate_bounds(
				before_bounds, dx, dy,
			)
...
def _align_selection_locally(app: object, direction: str) -> None:
	...
	objects_and_bounds = [
		(object_model, ferrum_qt.geometry.top_level_bounds(object_model))
...
def _align_selection(app: object, direction: str) -> None:
	...
	if session is not None and session.legacy_isolated:
		...
		return
	ferrum_qt.actions.top_level_transform_actions.submit_backend_transform(app, mode)

...
use crate::{
    LocalDocumentOpenCatalogV2, LocalDocumentOpenDescriptorV2, LocalDocumentOpenRouteV2,
    interchange_import_v1::InterchangeImportRefusalV1,
};

/// One closed local container kind carried by a prepared desktop admission.
#[derive(Clone, Copy)]
enum LocalDocumentSourceKindV2 {
    Catalog(&'static str),
}

impl LocalDocumentSourceKindV2 {
    const fn as_str(self) -> &'static str {
        match self {
            Self::Catalog(source_kind) => source_kind,
        }
    }
}

/// Immutable API-owned facts for one ordinary File/Open interchange route.
#[pyclass(frozen, module = "ferrum_chem", name = "LocalDocumentOpenDescriptorV2")]
pub(crate) struct PyLocalDocumentOpenDescriptorV2 {
    #[pyo3(get)]
    display_name: String,
    #[pyo3(get)]
    suffixes: Vec<String>,
    ...
    route_handle: Py<PyLocalDocumentOpenRouteHandleV2>,
}
...
#[pyclass(
    frozen,
    module = "ferrum_chem",
    name = "LocalDocumentOpenRouteHandleV2",
    skip_from_py_object
)]
pub(crate) struct PyLocalDocumentOpenRouteHandleV2 {
    descriptor: LocalDocumentOpenDescriptorV2,
}

#[pymethods]
impl PyLocalDocumentOpenDescriptorV2 {
    #[getter]
    fn route_handle(&self, py: Python<'_>) -> Py<PyLocalDocumentOpenRouteHandleV2> {
...
/// cannot inspect or mutate it until `take_admission_v2` transfers the session
/// and its authenticated observation exactly once.
#[pyclass(
    module = "ferrum_chem",
    name = "PreparedLocalDocumentOpenV2",
    skip_from_py_object
)]
...
#[pyclass(
    frozen,
    module = "ferrum_chem",
    name = "LocalDocumentOriginTokenV2",
    skip_from_py_object
)]
#[derive(Clone)]
pub(crate) struct PyLocalDocumentOriginTokenV2 {
    source: Arc<RetainedSourceFileGuardV1>,
}

#[pymethods]
impl PyLocalDocumentOriginTokenV2 {
...
impl PyDocumentSession {
    ...
    #[staticmethod]
    fn local_document_open_descriptors_v2(
        py: Python<'_>,
    ) -> PyResult<Vec<PyLocalDocumentOpenDescriptorV2>> {
        LocalDocumentOpenCatalogV2::snapshot()
            .map_err(|_| PyTypeError::new_err("local document open catalog is invalid"))?
            .into_iter()
            .map(|descriptor| {
                Ok(PyLocalDocumentOpenDescriptorV2 {
                    ...
                    suffixes: descriptor
                        ...
                        .collect(),
                    allows_current_tab_replacement: descriptor.allows_current_tab_replacement(),
                    route_handle: Py::new(py, PyLocalDocumentOpenRouteHandleV2 { descriptor })?,
                })
            })
            .collect()
    }

    /// Prepare one catalog-issued local source for a new document.
    #[staticmethod]
    fn prepare_local_document_open_file_v2(
        py: Python<'_>,
        path: &Bound<'_, PyAny>,
        route_handle: &Bound<'_, PyAny>,
    ) -> PyResult<PyPreparedLocalDocumentOpenV2> {
        prepare_local_document_open_file_v2(py, path, route_handle)
    }

    /// Read one catalog-issued interchange source as bounded UTF-8.
    ///
    ...
    #[staticmethod]
    fn read_local_interchange_utf8_v2(
        py: Python<'_>,
        path: &Bound<'_, PyAny>,
        route_handle: &Bound<'_, PyAny>,
    ) -> PyResult<String> {
        read_local_interchange_utf8_v2(py, path, route_handle)
    }
...
pub(crate) fn prepare_local_document_open_file_v2(
    py: Python<'_>,
    path: &Bound<'_, PyAny>,
    route_handle: &Bound<'_, PyAny>,
) -> PyResult<PyPreparedLocalDocumentOpenV2> {
    let path = exact_path(py, path)?;
    let descriptor = local_document_open_descriptor(route_handle)?;
    verify_catalog_path_suffix(py, &path, descriptor)?;
    match descriptor.route() {
        LocalDocumentOpenRouteV2::Cdml | LocalDocumentOpenRouteV2::DecodedCdsvg => {
            prepare_native_local_document_open_file_v2(py, path, descriptor)
        }
        LocalDocumentOpenRouteV2::Interchange(interchange_descriptor) => {
            prepare_interchange_local_document_open_file_v2(
                py,
                path,
                descriptor,
...
fn prepare_interchange_local_document_open_file_v2(
    py: Python<'_>,
    path: PathBuf,
    catalog_descriptor: LocalDocumentOpenDescriptorV2,
    descriptor: &'static crate::interchange_import_v1::InterchangeFormatDescriptorV1,
) -> PyResult<PyPreparedLocalDocumentOpenV2> {
    let source_kind = LocalDocumentSourceKindV2::Catalog(catalog_descriptor.route().source_kind());
    let result = py.detach(move || {
        let (source, prepared) =
    ...
    match result {
        Ok((session, observation, origin, source_kind, interchange_summary)) => {
            Ok(PyPreparedLocalDocumentOpenV2 {
                session: Some(session),
                observation: Some(super::render_binding::observation(py, observation)?),
                origin: Some(PyLocalDocumentOriginTokenV2 {
...
pub(crate) fn read_local_interchange_utf8_v2(
    py: Python<'_>,
    path: &Bound<'_, PyAny>,
    route_handle: &Bound<'_, PyAny>,
) -> PyResult<String> {
    let catalog_descriptor = local_document_open_descriptor(route_handle)?;
    let LocalDocumentOpenRouteV2::Interchange(descriptor) = catalog_descriptor.route() else {
        return Err(PyTypeError::new_err(
            "local document open route handle does not select an interchange source",
        ));
    };
    let path = exact_path(py, path)?;
    verify_catalog_path_suffix(py, &path, catalog_descriptor)?;
    let result = py.detach(move || {
...
fn local_document_open_descriptor(
    route_handle: &Bound<'_, PyAny>,
) -> PyResult<LocalDocumentOpenDescriptorV2> {
    let route_handle = exact_route_handle(route_handle)?;
    LocalDocumentOpenCatalogV2::snapshot()
        .map_err(|_| PyTypeError::new_err("local document open catalog is invalid"))?
        .into_iter()
        .find(|descriptor| descriptor.has_same_route(route_handle.descriptor))
        .ok_or_else(|| PyTypeError::new_err("local document open route handle is not API-issued"))
}
...
fn prepare_native_local_document_open_file_v2(
    py: Python<'_>,
    path: PathBuf,
    descriptor: LocalDocumentOpenDescriptorV2,
) -> PyResult<PyPreparedLocalDocumentOpenV2> {
    let result = py.detach(move || {
        let preparation = match descriptor.route() {
            LocalDocumentOpenRouteV2::Cdml => prepare_local_cdml_file_with_origin_v1(&path),
    ...
    match result {
        Ok((session, observation, origin)) => Ok(PyPreparedLocalDocumentOpenV2 {
            session: Some(session),
            observation: Some(super::render_binding::observation(py, observation)?),
            origin: Some(PyLocalDocumentOriginTokenV2 {
                source: Arc::new(origin),
            }),
            source_kind: Some(LocalDocumentSourceKindV2::Catalog(
                descriptor.route().source_kind(),
            )),
...
) -> PyResult<PyRef<'py, PyLocalDocumentOpenRouteHandleV2>> {
    if !route_handle.is_exact_instance_of::<PyLocalDocumentOpenRouteHandleV2>() {
        return Err(PyTypeError::new_err(
            "local document open route handle must be an API-issued handle",
        ));
    }
    Ok(route_handle.extract::<PyRef<'_, PyLocalDocumentOpenRouteHandleV2>>()?)
}

fn verify_catalog_path_suffix(
    py: Python<'_>,
    path: &std::path::Path,
    descriptor: LocalDocumentOpenDescriptorV2,
) -> PyResult<()> {
    ...
    if suffix.is_some_and(|suffix| descriptor.suffixes().contains(&suffix.as_str())) {
        Ok(())
    } else {
        Err(input_error(
            py,

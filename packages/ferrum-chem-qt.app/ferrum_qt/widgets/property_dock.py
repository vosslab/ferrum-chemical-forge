...
class PropertyDock(PySide6.QtWidgets.QDockWidget):
	...
	#============================================
	def set_document(
			self, document: ferrum_qt.models.document.Document | None,
	...
	def _set_bond_type_choices(self, current_type: str | None = None) -> None:
		...
		for type_char, label in ferrum_qt.bond_presentation.choices_for_display(current_type):
	...
	def _submit_atom_patch(self, changes: tuple[tuple[str, object], ...]) -> bool:
		...
		if not callable(self._atom_properties_capture):
			return False
		if not isinstance(self._current_item, ferrum_qt.canvas.items.atom_item.AtomItem):
			return True
		if self._document is None:
			return True
	...
	def _submit_bond_patch(self, changes: tuple[tuple[str, object], ...]) -> bool:
		...
		if not callable(self._bond_properties_capture):
			return False
		if not isinstance(self._current_item, ferrum_qt.canvas.items.bond_item.BondItem):
			return True

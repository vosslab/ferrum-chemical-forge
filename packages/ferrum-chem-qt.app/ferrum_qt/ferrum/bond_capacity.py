"""Read-only Ferrum selected-molecule bond-capacity report."""
...
#============================================
class FerrumNativeBondCapacityWorker(FerrumDetachedJobThread):
	...
	def __init__(self, observation: object, revision: int, digest: str,
		...
		super().__init__(
			lambda: engine.inspect_document_bond_capacity_v1(*self._arguments),
			lambda error: FerrumNativeBondCapacityFailure(str(error)),
		)

	#============================================
	def _emit_success(self, result: object) -> None:
		"""Retain the capacity route's established result signal."""
		self.completed.emit(result)


#============================================
class _BondCapacityDeliveryRelay(PySide6.QtCore.QObject):
	"""Deliver exact worker identity to its owning Ferrum window."""
...
#============================================
class FerrumNativeBondCapacityDialog(FerrumAccessibleDialog):
...
class FerrumNativeBondCapacityMixin:
	...
	def _on_document_bond_capacity_completed(self, worker: object, result: object) -> None:
		...
		if intent is None:
			return
		if (
			type(result) is not engine.DocumentBondCapacityV1
	...
	def _cancel_document_bond_capacity(self) -> None:
		"""Suppress late delivery while the bounded worker finishes."""
		intent = self._bond_capacity_intent
	...
	def _bond_capacity_blocks_tab_close(self, tab: object) -> bool:
		"""Keep the exact source tab live until the current operation ends."""
		intent = self._bond_capacity_intent
		if intent is None or intent.tab is not tab:
			return False
		self._show_edit_refusal(self._unavailable_edit_refusal("Cancel the check and wait for the current operation before closing."))
		return True

"""Focused Ferrum atom-number dialog with Rust document authority."""
...
#============================================
class FerrumNativeAtomNumberDialog(FerrumAccessibleDialog):
	...
	def __init__(self, number: int | None, show_number: bool | None,
		...
		if number is not None and (type(number) is not int or number <= 0):
			raise ValueError("Ferrum atom number must be a positive integer")
		if show_number is not None and type(show_number) is not bool:
			raise TypeError("Ferrum atom number visibility must be a bool")

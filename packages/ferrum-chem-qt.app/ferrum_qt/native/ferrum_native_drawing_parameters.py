"""Application-owned next-operation choices for native drawing tools."""
...
class FerrumNativeDrawingParametersSnapshot:
	"""Frozen user choices carried by one future native authoring gesture."""
	...
	def bond_order(self) -> object:
		...
		if self.order_name == "single":
			return ferrum_chem.DocumentBondOrderV1.single
		if self.order_name == "double":
			return ferrum_chem.DocumentBondOrderV1.double
		if self.order_name == "triple":
			return ferrum_chem.DocumentBondOrderV1.triple
		raise ValueError("native drawing parameters contain an unknown bond order")

	#============================================
	def bond_presentation(self) -> object:
		"""Convert frozen next-drawing choices at the private PyO3 boundary."""
		import ferrum_chem
		presentations = ferrum_chem.DocumentBondPresentationV1
		...
		raise ValueError("native drawing parameters contain an unknown presentation")
...
def _periodic_spellings_by_casefold() -> dict[str, str]:
	...
	return {
		...
		for entry in ferrum_chem.periodic_display_entries_v1()
	}
...
class FerrumNativeDrawingParameters(PySide6.QtCore.QObject):
	...
	def shared_application_model(cls, preferences: object) -> "FerrumNativeDrawingParameters":
		...
		if cls._shared_application_model is None:
			raise RuntimeError("native drawing parameters have no shared application model")
...
def common_elements() -> tuple[str, ...]:
	"""Return the authoritative periodic picker suggestions for native drawing."""
	import ferrum_chem
	return tuple(entry.symbol for entry in ferrum_chem.periodic_display_entries_v1())

...
class ExportProjection:
	...
	def dispose(self) -> None:
		...
		if scene is None:
			return
		from ferrum_qt.canvas.graphics_retirement import temporary_scene_retirement_reaper
...
def _project_marks(
	...
	for source_item in source_scene.items():
		...
		item = ferrum_qt.canvas.document_projection.create_mark_item(mark_model, parent)
		if item is None:
			continue
	...
	try:
		for molecule in molecules:
			...
			for atom_model in molecule.atoms:
				item = ferrum_qt.canvas.items.atom_item.AtomItem(atom_model)
				detached_items.append(item)
				item.molecule_model = molecule
		...
		for presentation in presentations:
			item = ferrum_qt.canvas.document_projection.create_presentation_item(presentation)
			if item is None:
				continue
			detached_items.append(item)

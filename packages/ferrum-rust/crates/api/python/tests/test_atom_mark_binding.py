...
def test_live_atom_element_position_and_deletion_lower_only_valid_durable_targets() -> None:
	...
	changed = session.set_atom_element_v1(
		snapshot.revision, snapshot.digest, molecule.id, atom.id, "N",
	)
...
def test_live_atom_adapters_reject_stale_revision_and_digest_without_mutation() -> None:
	...
	with pytest.raises(ferrum_chem.RevisionConflictError) as revision_error:
		session.set_atom_element_v1(
			before.revision + 1, before.digest, molecule.id, atom.id, "N",
		)
	with pytest.raises(ferrum_chem.RevisionConflictError):
		session.set_atom_position_v1(
			before.revision + 1, before.digest, molecule.id, atom.id, 3.0, 4.0, 0.0,
		)
	with pytest.raises(ferrum_chem.RevisionConflictError):
		session.delete_atom_v1(
			before.revision + 1, before.digest, molecule.id, atom.id,
		)
	...
	with pytest.raises(ferrum_chem.RevisionConflictError) as digest_error:
		session.set_atom_element_v1(
			before.revision, "0" * 64, molecule.id, atom.id, "N",
		)
	with pytest.raises(ferrum_chem.RevisionConflictError):
		session.set_atom_position_v1(
			before.revision, "0" * 64, molecule.id, atom.id, 3.0, 4.0, 0.0,
		)
	with pytest.raises(ferrum_chem.RevisionConflictError):
		session.delete_atom_v1(
			before.revision, "0" * 64, molecule.id, atom.id,
		)
...
def test_live_atom_adapters_refuse_invalid_durable_targets_without_mutation() -> None:
	...
	with pytest.raises(ferrum_chem.OperationValidationError):
		session.set_atom_element_v1(before.revision, before.digest, first.id, foreign_atom.id, "N")
	with pytest.raises(ferrum_chem.OperationValidationError):
		session.set_atom_position_v1(
			before.revision, before.digest, first.id, foreign_atom.id, 3.0, 4.0, 0.0,
		)
	with pytest.raises(ferrum_chem.OperationValidationError):
		session.delete_atom_v1(before.revision, before.digest, first.id, foreign_atom.id)

	with pytest.raises(ferrum_chem.OperationValidationError):
		session.set_atom_element_v1(before.revision, before.digest, first.id, bond.id, "N")
	with pytest.raises(ferrum_chem.OperationValidationError):
		session.set_atom_position_v1(
			before.revision, before.digest, first.id, bond.id, 3.0, 4.0, 0.0,
		)
	with pytest.raises(ferrum_chem.OperationValidationError):
		session.delete_atom_v1(before.revision, before.digest, first.id, bond.id)
	assert (session.snapshot().revision, session.snapshot().digest, session.snapshot().cdml) == (
		before.revision, before.digest, before.cdml,
	)
...
def test_live_atom_adapters_refuse_missing_and_invalid_operation_input() -> None:
	...
	with pytest.raises(ferrum_chem.InvalidAtomElementError):
		session.set_atom_element_v1(before.revision, before.digest, first.id, atom.id, "Xx")
	with pytest.raises(ferrum_chem.ProjectionError):
		session.set_atom_position_v1(
			before.revision, before.digest, first.id, atom.id, float("nan"), 4.0, 0.0,
		)
	assert (session.snapshot().revision, session.snapshot().digest, session.snapshot().cdml) == (
		before.revision, before.digest, before.cdml,
	)
	...
	deleted = session.delete_atom_v1(
		before.revision, before.digest, first.id, atom.id,
	)
	current = deleted.observation.snapshot
	with pytest.raises(ferrum_chem.UnknownDocumentObjectError):
		session.set_atom_element_v1(current.revision, current.digest, first.id, atom.id, "N")
	with pytest.raises(ferrum_chem.UnknownDocumentObjectError):
		session.set_atom_position_v1(
			current.revision, current.digest, first.id, atom.id, 3.0, 4.0, 0.0,
		)
	with pytest.raises(ferrum_chem.UnknownDocumentObjectError):
		session.delete_atom_v1(current.revision, current.digest, first.id, atom.id)
	assert (session.snapshot().revision, session.snapshot().digest, session.snapshot().cdml) == (

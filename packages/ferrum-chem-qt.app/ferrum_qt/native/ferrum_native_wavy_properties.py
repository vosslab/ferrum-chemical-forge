"""Typed Wavy appearance editing for the OASA-free native window."""
...
def dialog_model_from_projection(root: object) -> FerrumNativeWavyDialogModel:
	...
	if type(root) is not ferrum_chem.PresentationRootProjectionV1:
		raise TypeError("native Wavy properties require an exact Ferrum root")
	if root.kind != "wavy" or type(root.polyline) is not ferrum_chem.PolylineProjectionV1:
	...
	if (
		...
		raise ValueError("native Wavy properties require a durable authored target")
...
def property_changes_from_dialog(
	...
	if type(changes) is not tuple:
		raise TypeError("native Wavy property changes must be an exact tuple")
	import ferrum_chem
	converted: list[object] = []
	for change in changes:
		if type(change) is not tuple or len(change) != 2:
			raise TypeError("native Wavy property changes must be exact field/value pairs")
		field, value = change
		if field == "width" and type(value) is float:
			converted.append(
				ferrum_chem.DocumentWavyPropertyChangeV1.line_width(value),
			)
		elif field == "line_color" and type(value) is str:
			converted.append(
				ferrum_chem.DocumentWavyPropertyChangeV1.line_color(value),
			)
		else:
			raise ValueError("native Wavy form supplied an unsupported property change")
...
class FerrumNativeWavyPropertiesMixin:
	...
	def apply_selected_wavy_properties(self, expected_target_id: str,
		...
		if type(expected_target_id) is not str or type(expected_source_id) is not str:
			raise TypeError("native Wavy properties require durable string identifiers")
		if type(changes) is not tuple:
			raise TypeError("native Wavy properties require an exact change tuple")
		...
		import ferrum_chem
		if any(type(change) is not ferrum_chem.DocumentWavyPropertyChangeV1
				for change in changes):
			raise TypeError("native Wavy properties require exact frozen Ferrum changes")
...
def install_wavy_properties_action(window: object,
	...
	"""Install one native Wavy-properties action on the public window."""
	action = PySide6.QtGui.QAction(window.tr("Edit Wavy Properties"), window)
	action.setToolTip(window.tr(
		"Edit one selected Wavy line through one Rust-native operation",
	))
...
def _on_edit_wavy_properties(window: object) -> None:
	...
	try:
		...
		window._show_native_file_warning("Native Wavy Properties Unavailable", str(exc))
		return
	...
	try:
		...
		window._refresh_actions()
		window._show_native_file_warning("Native Wavy Properties Error", str(exc))
		return
	window.statusBar().showMessage(window.tr("Updated one Rust-native Wavy line."), 5000)
	window._refresh_actions()

...
import ferrum_qt.widgets.submode_ribbon
...
def setup_toolbars(
	...
	ferrum_qt.widgets.icon_loader.set_theme(theme_manager.current_theme)
	...
	if not modes_yaml_path.is_file():
		raise FileNotFoundError(
			...
			"Reinstall the ferrum-qt package."
		)
	...
	mode_toolbar = ferrum_qt.widgets.mode_toolbar.ModeToolbar(window)
	registered_modes = set(mode_manager.mode_names())

	# keybindings for shortcut tooltips
	keybindings = ferrum_qt.config.keybindings.DEFAULT_KEYBINDINGS
	...
	submode_ribbon = ferrum_qt.widgets.submode_ribbon.SubModeRibbon(window)
	...
	window.addToolBarBreak()
	edit_ribbon = ferrum_qt.widgets.edit_ribbon.EditRibbon(window)
	...
	property_dock = ferrum_qt.widgets.property_dock.PropertyDock(

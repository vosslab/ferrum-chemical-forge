"""Private Rust regular-ring session seam for a native document tab."""
...
class FerrumNativeRegularRingTabMixin:
	...
	def prepare_regular_ring(self, size: int, center_x: float, center_y: float,
		...
		return self._session.prepare_create_regular_ring_v1(
	...
	def commit_regular_ring(self, prepared: object) -> object:
		self._require_mutable()
		import ferrum_chem
		if type(prepared) is not ferrum_chem.PreparedRegularRingInsertionV1:
			raise TypeError("native regular ring requires an exact Rust prepared receipt")
		result = self._session.commit_create_regular_ring_v1(

...
class FerrumNativeSnapshotFormat(enum.Enum):
	"""Closed complete-document artifact formats for the Ferrum window."""
...
class _SnapshotExportFailure:
	"""One user-safe Ferrum preparation failure delivered to the Qt thread."""

	category: str


#============================================
class _SnapshotExportWorker(FerrumDetachedJobThread):
	...
	def __init__(self, capture: _SnapshotExportCapture,
		...
		if type(capture.observation) is not engine.SessionDocumentObservationV1:
			raise TypeError("Ferrum artifact export requires an exact Ferrum observation")
		self._capture = capture
		self._profile = _rust_profile(export_format)
		super().__init__(
			lambda: engine.prepare_document_native_artifact_v1(
				self._capture.observation, self._capture.revision,
				self._capture.digest, self._profile,
			),
			_snapshot_export_failure,
		)

	#============================================
	def _emit_success(self, receipt: object) -> None:
		"""Retain the artifact route's established prepared signal."""
		self.prepared.emit(receipt)


#============================================
def _snapshot_export_failure(error: Exception) -> _SnapshotExportFailure:
	"""Map artifact errors without retaining their worker-thread instances."""
	if type(error) is engine.DocumentNativeArtifactError:
		category = getattr(error, "category", "preparation_failed")
		return _SnapshotExportFailure(
			category if type(category) is str else "preparation_failed",
		)
	return _SnapshotExportFailure("preparation_failed")


#============================================
class _SnapshotExportDeliveryRelay(PySide6.QtCore.QObject):
	"""Return Ferrum artifact worker events to their owning window."""
...
class FerrumNativeSnapshotExportWindowMixin:
	...
	def _initialize_snapshot_exports(self) -> None:
		"""Initialize the single Ferrum artifact export intent."""
	...
	def _snapshot_export_busy(self) -> bool:
		"""Return whether one Ferrum artifact receipt remains in flight."""
	...
	def export_active_snapshot(
		...
		if type(path) is not str or type(export_format) is not FerrumNativeSnapshotFormat:
			raise TypeError("Ferrum artifact export requires an exact path and format")
		capture = self._capture_snapshot_export()
		if capture is None:
			return False
	...
	def _capture_snapshot_export(self) -> _SnapshotExportCapture | None:
		...
		try:
			...
			self._show_edit_refusal(self._unavailable_edit_refusal("The current document is not ready for export. Refresh it and try again."))
			return None
		snapshot = tab.current_snapshot
		observed = observation.snapshot
		if observed.revision != snapshot.revision or observed.digest != snapshot.digest:
			self._show_edit_refusal(self._unavailable_edit_refusal("The current document changed before export could begin. Choose Export again."))
			return None
	...
	def _normalize_snapshot_export_path(
		...
		if not path.suffix:
			...
			self._show_edit_refusal(self._unavailable_edit_refusal("Ferrum %s exports must use the .%s extension."
	...
	def start_document_snapshot_export(
		...
		if type(capture) is not _SnapshotExportCapture:
			raise TypeError("Ferrum artifact export requires an exact document capture")
		if type(destination) is not str or not os.path.isabs(destination):
			raise ValueError("Ferrum artifact export requires an absolute destination")
		if type(export_format) is not FerrumNativeSnapshotFormat:
			raise TypeError("Ferrum artifact export requires an exact closed format")
		if not self._snapshot_capture_is_current(capture):
			return False
		try:
	...
	def _on_snapshot_export_prepared(self, worker: object, receipt: object) -> None:
		...
		if intent is None:
			self._show_stale_snapshot_export()
			return
		if type(receipt) is not engine.PreparedDocumentNativeArtifactV1:
			self._show_edit_refusal(self._unavailable_edit_refusal("Ferrum returned an unexpected prepared artifact. No successful export "
				"is being reported."))
			return
		if (
			...
			return
		try:
			publication = engine.publish_prepared_document_native_artifact_v1(
				...
				engine.PublicationPossiblyCompletedError,
			) as error:
			self._report_snapshot_publication_error(intent, error)
			return
		if type(publication) is not engine.DocumentNativeArtifactPublicationV1:
			self._show_edit_refusal(self._unavailable_edit_refusal("Ferrum returned an unexpected publication value. Inspect %s because Rust "
		...
		if publication.directory_entry_confirmed:
			...
			return
		self._show_edit_refusal(self._unavailable_edit_refusal("The exact %s artifact may be present at %s, but directory-entry durability "
	...
	def _cancel_snapshot_export(self) -> None:
		...
		self.statusBar().showMessage(
			self.tr("Cancelling export; waiting for the current operation to finish..."), 0,
		)
		self._refresh_actions()
	...
	def _snapshot_export_blocks_tab_close(self, tab: object) -> bool:
		...
		self._show_edit_refusal(self._unavailable_edit_refusal("Ferrum cancelled delivery; close this tab again after the current operation finishes."))
		return True

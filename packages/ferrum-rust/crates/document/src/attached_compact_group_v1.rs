//! Closed methyl-only compact-group attachment facts and typed-CDML mutation.
...
#[derive(Clone, Copy, Debug, PartialEq)]
pub(crate) struct AttachedMethylCandidateV1 {
    anchor: Point3V1,
    catalog_key: CompactGroupCatalogKeyV1,
    attachment: CompactGroupAttachmentV1,
}

impl AttachedMethylCandidateV1 {
...
/// Derive the only accepted persisted compact-group pose from the direct release vector.
pub(crate) fn attached_methyl_candidate_v1(
    anchor: Point3V1,
    release: AttachedCompactGroupReleaseV1,
) -> Result<AttachedMethylCandidateV1, AttachedCompactGroupErrorV1> {
    ...
    let attachment = CompactGroupAttachmentV1::new(
        catalog_key,
        0,
        dy.atan2(dx).to_degrees(),
    )
    .map_err(|_| AttachedCompactGroupErrorV1::InvalidPose)?;
    Ok(AttachedMethylCandidateV1 {
        anchor: group_anchor,
        catalog_key,
        attachment,
    })
...
impl TypedDocument {
    /// Build exactly one direct methyl group plus one normal exterior bond.
    pub(crate) fn with_attach_methyl_compact_group_v1(
        &self,
        molecule_id: &PersistentId,
        anchor_atom_id: &PersistentId,
        group_id: &PersistentId,
        bond_id: &PersistentId,
        candidate: AttachedMethylCandidateV1,
        ...
        let root = tree.document_element(indexed.xml.document).expect("parsed CDML has a root");
        ...
        let anchor_exists = tree.children(molecule).any(|node| {
            element_name(tree, node).is_some_and(|(local, node_namespace)| {
                local == "atom" && node_namespace == CDML_NAMESPACE
                    && tree.get_attribute(node, id_name) == Some(anchor_atom_id.as_str())
            })
        });
        if !anchor_exists {
            return Err(TypedDocumentError::InvalidBondEndpoint(anchor_atom_id.clone()));
        }
        ...
        for (name, value) in [
            ("id", bond_id.as_str()), ("start", anchor_atom_id.as_str()),
            ("end", group_id.as_str()), ("type", "n1"),
        ] {
            let attribute = tree.add_name(name);
            tree.set_attribute(bond, attribute, value);
        }
        tree.append(molecule, group).map_err(TypedDocumentError::Mutation)?;
        tree.append(molecule, bond).map_err(TypedDocumentError::Mutation)?;
...
mod tests {
    ...
    #[test]
    fn methyl_candidate_derives_a_finite_canonical_pose() {
        let anchor = Point3V1::new(0.0, 0.0, 0.0).expect("anchor");
        let candidate = attached_methyl_candidate_v1(
            anchor,
            AttachedCompactGroupReleaseV1::new(20.0, 0.0).expect("release"),
        )
        .expect("candidate");
        assert_eq!(candidate.anchor(), Point3V1::new(20.0, 0.0, 0.0).expect("point"));
    ...
    #[test]
    fn attached_methyl_reports_the_colliding_group_identifier() {

...
def _build_arrow(root: object, extension: object) -> ArrowProjectionItem:
	"""Build one normal arrow entirely from authenticated Rust display geometry."""
	if any(value is not None for value in (
	...
	axis_points = _arrow_points(arrow.axis_path, extension, "axis")
	...
	if type(arrow.heads) is not list:
	...
	if any(type(head) is not extension.ArrowHeadV1 for head in arrow.heads):
		raise PresentationProjectionError("arrow head has the wrong DTO type")
	if [head.position for head in arrow.heads] != expected_positions:
	...
	axis_path = PySide6.QtGui.QPainterPath(axis_points[0])

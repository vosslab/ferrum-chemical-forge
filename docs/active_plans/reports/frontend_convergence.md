...
| --- | --- | --- |
| Resources, menus, and modes | Historical `resources/menus.yaml` declares menu hierarchy separately from callbacks; `f3a6b2f` also adjusted `resources/modes.yaml` for responsive tools. These read-only non-shipping reference paths are intentionally plain text. | [`menus.yaml`](../../../packages/ferrum-chem-qt.app/ferrum_qt/resources/menus.yaml) and [`modes.yaml`](../../../packages/ferrum-chem-qt.app/ferrum_qt/resources/modes.yaml) retain that declarative hierarchy pattern after mapping only supported command and tool IDs. [`test_frontend_portability_contract.py`](../../../packages/ferrum-chem-qt.app/tests/test_frontend_portability_contract.py) asserts the shipped IDs and ordered menu/mode shape. |

...
class AddMoleculeCommand(PySide6.QtGui.QUndoCommand):
	...
	def redo(self) -> None:
		...
		if not all(
				ferrum_qt.canvas.graphics_retirement.can_add_item_to_captured_scene(
					self._scene, item,
	...
	def undo(self) -> None:
		...
		if not all(
				ferrum_qt.canvas.graphics_retirement.item_belongs_to_scene(
					self._scene, item,
				) for item in self._graphics_items
				):
			return
...
class AddPresentationObjectCommand(PySide6.QtGui.QUndoCommand):
	...
	def redo(self) -> None:
		...
		if not self._document.is_current_projection_scene(self._scene):
			return
		if not ferrum_qt.canvas.graphics_retirement.can_add_item_to_captured_scene(
	...
	def undo(self) -> None:
		...
		if not self._document.is_current_projection_scene(self._scene):
			return
		if not ferrum_qt.canvas.graphics_retirement.item_belongs_to_scene(
				self._scene, self._graphics_item,
				):
			return
		ferrum_qt.canvas.graphics_retirement.remove_item_from_captured_scene(
...
class AddAtomMarkCommand(PySide6.QtGui.QUndoCommand):
	...
	def redo(self) -> None:
		...
		scene = ferrum_qt.canvas.graphics_retirement.native_scene_for_item(
			self._parent_atom_item,
		)
		if not ferrum_qt.canvas.graphics_retirement.set_item_parent_in_captured_scene(

"""Mach-O closure assembly and validation for Ferrum's local native runtime.
...
class NativeMachoError(RuntimeError):
	"""An actionable failure in the staged local Mach-O runtime closure."""
...
#============================================
def copy_and_rewrite_runtime_closure(
	adapter: Path,
	...
	for library in closure(seeds):
		destination = runtime_libs / library.name
		if destination.exists():
			raise NativeMachoError(
				f"duplicate dependency basename in closure: {library.name}"
			)
		shutil.copy2(library, destination)
	for library in runtime_libs.glob("*.dylib"):
		...
		for dependency in otool_dependencies(library):
			...
			if not (runtime_libs / name).is_file():
				raise NativeMachoError(
...
# Local-runtime closure validation ---------------------------------------------
...
#============================================
def validate_runtime_dylib_closure(
	identity: str,
	...
	"""Validate one staged runtime dylib's identity, rpaths, and non-system loads."""
	if name not in allowed_non_system_names:
		raise NativeMachoError(
			f"runtime dylib is outside the Ferrum platform closure allowlist: {name}"
		)
	if identity != f"@loader_path/{name}":
		raise NativeMachoError(
			f"runtime dylib has a non-runtime identity: {name} -> {identity}"
		)
	...
	for dependency in dependencies:
		...
		if dependency_name not in allowed_non_system_names:
			raise NativeMachoError(
			f"runtime dylib has forbidden native dependency: {name} -> {dependency}"
			)
		if dependency != expected or dependency_name not in runtime_names:
			raise NativeMachoError(
				"runtime dylib has an unbundled or non-loader-relative dependency: "
				f"{name} -> {dependency}"
			)
...
def validate_extension_closure(
	...
	if not has_adapter:
		raise NativeMachoError("extension rpath has no staged libferrum_chem.dylib target")


#============================================
def assert_runtime_dylib(binary: Path, runtime_libs: Path) -> None:
	"""Inspect one staged runtime dylib against the frozen platform closure."""
	validate_runtime_dylib_closure(
		otool_identity(binary),
		otool_dependencies(binary),
		otool_rpaths(binary),
		binary.name,
		{library.name for library in runtime_libs.glob("*.dylib")},
		MACOS_ARM64_NATIVE_CLOSURE.allowed_non_system_names,
	)


#============================================
def assert_runtime_library_closure(
	runtime_libs: Path,
	profile: RdkitCapabilityProfile = FERRUM_RDKIT_PROFILE,
) -> None:
	"""Reject a local runtime closure that differs from Ferrum's platform policy."""
	runtime_names = {library.name for library in runtime_libs.glob("*.dylib")}
	if runtime_names != MACOS_ARM64_NATIVE_CLOSURE.allowed_non_system_names:
		raise NativeMachoError(
			"local native runtime closure differs from frozen Ferrum platform profile: "
			f"expected {sorted(MACOS_ARM64_NATIVE_CLOSURE.allowed_non_system_names)}, "
			f"got {sorted(runtime_names)}"
		)
	for name in runtime_names:
		if any(fragment in name.lower() for fragment in profile.forbidden_native_fragments):
			raise NativeMachoError(f"forbidden native library in local runtime closure: {name}")
	for library in sorted(runtime_libs.glob("*.dylib")):
		assert_runtime_dylib(library, runtime_libs)


#============================================
def assert_clean_runtime_closure(
	extension: Path,
	runtime_libs: Path,
	profile: RdkitCapabilityProfile = FERRUM_RDKIT_PROFILE,
) -> None:
	"""Reject a runtime closure that differs from Ferrum's frozen macOS profile."""
	assert_runtime_library_closure(runtime_libs, profile)
	validate_extension_closure(
		otool_dependencies(extension),
		otool_rpaths(extension),
		(runtime_libs / "libferrum_chem.dylib").is_file(),
	)
...
def self_test() -> None:
	...
	reject(
		lambda: validate_runtime_dylib_closure(

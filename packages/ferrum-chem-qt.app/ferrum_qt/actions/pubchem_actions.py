"""Explicit PubChem lookup and insertion actions for Ferrum-Qt."""
...
class PubChemLookupDialog(PySide6.QtWidgets.QDialog):
	...
	def set_lookup_result(
			self,
			prepared: ferrum_qt.bridge.chemistry_preparation.PreparedPubChemLookup,
...
class _PubChemLookupRelay(PySide6.QtCore.QObject):
	...
	def on_result(self, prepared: object) -> None:
		...
		if not self._is_current():
			return
		if not ferrum_qt.bridge.chemistry_preparation.is_prepared_pubchem_lookup(prepared):
...
def _insert_dialog_result(app: object, dialog: PubChemLookupDialog) -> bool:
	...
	if (
			dialog._closed
			or not ferrum_qt.bridge.chemistry_preparation.is_prepared_pubchem_lookup(prepared)
			):
		return False
	if (
	...
	if (
		...
		return False
	insertion = ferrum_qt.bridge.chemistry_preparation.molecule_insertion_proposal(
		prepared.insertion,
	)
...
def open_pubchem_lookup(
		app: object,
		transport: object = ferrum_qt.bridge.chemistry_preparation.fetch_pubchem_json,
		) -> PubChemLookupDialog:

...
def create_canvas(
	...
	view = ferrum_qt.canvas.view.ChemView(scene, parent=view_parent)
	...
	surround = ferrum_qt.themes.theme_loader.get_canvas_surround(theme)
...
def _apply_theme_colors(theme: str) -> None:
	...
	chem_colors = ferrum_qt.themes.theme_loader.get_chemistry_colors(theme)
	ferrum_qt.canvas.items.render_ops_painter.set_default_color(
		chem_colors["default_line"]
	)
	# set default area color for atom label background masking
	ferrum_qt.canvas.items.render_ops_painter.set_default_area_color(
		chem_colors["default_area"]
	)
	# set canvas interaction colors from YAML theme
	canvas_colors = ferrum_qt.themes.theme_loader.get_canvas_colors(theme)
	ferrum_qt.canvas.items.render_ops_painter.set_canvas_colors(canvas_colors)
	# set charge mark colors from YAML theme
	ferrum_qt.canvas.items.render_ops_painter.set_charge_colors({
		"plus": chem_colors["charge_plus"],
		"minus": chem_colors["charge_minus"],
	})
	# set the light theme sentinel for dark mode color remapping
	light_chem = ferrum_qt.themes.theme_loader.get_chemistry_colors("light")
	ferrum_qt.canvas.items.render_ops_painter.set_light_default_line(

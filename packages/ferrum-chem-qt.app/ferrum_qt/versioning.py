...
def _source_tree_version() -> str | None:
	...
	if (
		os.path.basename(package_root) != "ferrum-chem-qt.app"
		or os.path.basename(packages_dir) != "packages"
	):
		return None

	version_path = os.path.join(os.path.dirname(packages_dir), "VERSION")
	return ferrum_qt.bridge.release_metadata.read_source_tree_display_version(version_path)


#============================================
def application_version() -> str:
	"""Return the exact Ferrum CalVer display label in every supported layout."""
	source_version = _source_tree_version()
	if source_version is not None:
		return source_version
	return ferrum_qt.bridge.release_metadata.installed_display_version("ferrum-qt")

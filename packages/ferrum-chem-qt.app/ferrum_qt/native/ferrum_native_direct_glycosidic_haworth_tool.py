"""Ordinary Qt request and one-shot placement for native direct-glycosidic Haworth."""
...
import ferrum_qt.native.ferrum_native_direct_glycosidic_haworth
from ferrum_qt.native.ferrum_native_document_tab_errors import (
...
class _DirectGlycosidicHaworthDialog(PySide6.QtWidgets.QDialog):
...
class FerrumNativeDirectGlycosidicHaworthWindowMixin:
	...
	def _refresh_direct_glycosidic_haworth_action(self, active: bool,
		...
		"""Retire a source-bound request once its native source is no longer current."""
	...
	def _on_insert_direct_glycosidic_haworth(self, _checked: bool = False) -> bool:
		...
		while dialog.exec() == PySide6.QtWidgets.QDialog.DialogCode.Accepted:
			...
			except ferrum_chem.DirectHaworthError as error:
	...
	def _place_direct_glycosidic_haworth(self,
		...
		except ferrum_chem.DirectHaworthError:
			self._cancel_direct_glycosidic_haworth_intent()
			self._show_native_file_warning("Direct-Glycosidic Haworth Error", self.tr(
				"The drawing could not be placed. Choose Insert Direct-Glycosidic Haworth "
				"again and try another empty page location.",
			))
			return
		try:
			preview = ferrum_qt.native.ferrum_native_direct_glycosidic_haworth.create_preview(
				intent.tab, prepared,
			)
		except ValueError:
			self._cancel_direct_glycosidic_haworth_intent()
			self._show_native_file_warning("Direct-Glycosidic Haworth Preview Error", self.tr(
		...
		try:
			...
			self._cancel_direct_glycosidic_haworth_intent()
			self._show_native_file_warning("Direct-Glycosidic Haworth Needs Refresh", self.tr(

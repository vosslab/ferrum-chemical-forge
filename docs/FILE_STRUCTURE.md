...
- [../packages/ferrum-rust/crates/render/](../packages/ferrum-rust/crates/render/)
  ...
  `assets/fonts/catalog.json` manifest inventories every vendored Atkinson
  Hyperlegible Next and Mono binary; `devel/verify_vendored_font_catalog.py`
  verifies that offline inventory explicitly.
...
- `packages/ferrum-rust/crates/render/assets/fonts/` contains the cataloged
  Atkinson Hyperlegible Next and Mono font binaries used for verification and
  molecule-label rendering. Its two license files live in `assets/licenses/`;
  these are source assets, not generated output.
...
- [../packages/ferrum-chem-qt.app/ferrum_qt/canvas/](../packages/ferrum-chem-qt.app/ferrum_qt/canvas/)
  ...
  `molecule_label_font.py` replays the verified Rust-selected molecule-label
  face.
...
- `build/.cargo-target-*`, `build/cargo-target/`, the Rust workspace `target/`,
  and `output_native_wheel/` are build-owned intermediates. [../build.sh](../build.sh)
  removes its fixed owned paths after publishing a local program.
- Python packaging output uses ignored `dist/`, `sdist/`, `site/`, and
  `*.egg-info/` paths.
- Root `output*/` paths hold disposable developer and release evidence. Examples
  include glyph-alignment output, Qt measurement captures, and release archives;
  their producing scripts create or replace them instead of treating them as
  source assets.
...
- `packages/ferrum-chem-qt.app/ferrum_qt/resources/` contains tracked packaged
  YAML, SVG, and light/dark PNG interface assets. Add product resources there
  only when the Qt package manifest includes them.

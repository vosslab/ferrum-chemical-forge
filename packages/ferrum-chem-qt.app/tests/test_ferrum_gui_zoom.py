"""Ferrum-Qt GUI zoom behavior tests."""
...
def _simulate_snapped_button_zoom(start_percent: float, direction: int, steps: int) -> float:
	...
	levels = ferrum_qt.canvas.view.ZOOM_SNAP_LEVELS
	for _idx in range(steps):
		if direction > 0:
			raw = current * ferrum_qt.canvas.view.ZOOM_FACTOR_PER_NOTCH
		else:
			raw = current / ferrum_qt.canvas.view.ZOOM_FACTOR_PER_NOTCH
		raw = max(
			ferrum_qt.canvas.view.ZOOM_MIN_PERCENT,
			min(raw, ferrum_qt.canvas.view.ZOOM_MAX_PERCENT),
		)
		target = min(levels, key=lambda level: (abs(level - raw), -level))
		if direction > 0 and target <= current and current < ferrum_qt.canvas.view.ZOOM_MAX_PERCENT:
			for level in levels:
				if level > current:
					target = level
					break
		elif direction < 0 and target >= current and current > ferrum_qt.canvas.view.ZOOM_MIN_PERCENT:
...
def test_qt_gui_zoom_diagnostic(main_window: object) -> None:
	...
	assert zoom_after_content <= ferrum_qt.canvas.view.ZOOM_MAX_PERCENT
	assert any(

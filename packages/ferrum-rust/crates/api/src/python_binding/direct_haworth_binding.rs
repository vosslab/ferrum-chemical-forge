//! Python source and resolver seam for direct-glycosidic Haworth authoring.

use ferrum_chemistry::{ChemistryError, NativeChemEngine};
use ferrum_document::{DocumentSessionError, Point3V1};
use ferrum_domain::haworth::{
    DirectGlycosidicHaworthAuthoringReceiptV1, DirectHaworthFromSmilesBuildErrorV1,
...
use super::{
    binding::PyDocumentSession, prepared_transition_binding::PySessionOperationTransitionRequestV1,
};
...
/// Immutable parsed direct-Haworth source, before document transition preparation.
#[pyclass(
    frozen,
    unsendable,
    module = "ferrum_chem",
    name = "DirectHaworthSourceV1"
)]
pub(crate) struct PyDirectHaworthSourceV1 {
...
pub(crate) fn prepare_direct_haworth_from_smiles_v1(
    py: Python<'_>,
    smiles: &str,
) -> PyResult<PyDirectHaworthSourceV1> {
    ...
    match result {
        Ok(prepared) => Ok(PyDirectHaworthSourceV1 {
...
impl PyDocumentSession {
    /// Bind parsed Haworth source and one anchor to generic transition authority.
    fn resolve_direct_haworth_transition_v1(
        &self,
        py: Python<'_>,
        expected_revision: u64,
        source: PyRef<'_, PyDirectHaworthSourceV1>,
        anchor_x: f64,
        anchor_y: f64,
        ...
        self.session
            .resolve_direct_haworth_transition_v1(expected_revision, source.receipt.clone(), anchor)
    ...
    match error {
        ...
        PreparationFailure::Build(error) => match error {
            ...
            | DirectHaworthFromSmilesBuildErrorV1::Identifier(_) => receipt_error(
                py,
                "direct Haworth preparation could not create a checked source",
            ),
        },
    }
}
...
fn map_session_error(py: Python<'_>, error: DocumentSessionError) -> PyErr {
    match error {
        ...
        _ => receipt_error(py, "the direct Haworth document operation is unavailable"),
    }
}

"""Native actions for Rust-owned molecule geometry repair."""
...
# local repo modules
import ferrum_qt.native.ferrum_native_coordinate_generation as native_coordinates
...
class FerrumNativeGeometryRepairTabMixin:
	...
	def selected_geometry_repair_molecules(
		...
		if self._document_observation is None:
			raise _tab_error("native tab has no installed document projection")
	...
	def selected_clean_geometry_molecules(
			self) -> tuple[tuple[str, ...], tuple[tuple[str, str], ...]]:
		"""Return durable document-object selectors for native preparation."""
	...
	def repair_geometry_at_revision(
		...
		if type(expected_revision) is not int:
			raise TypeError("native geometry repair requires an exact revision")
		if type(molecule_ids) is not tuple or any(type(value) is not str for value in molecule_ids):
			raise TypeError("native geometry repair requires an exact tuple of molecule IDs")
		if type(restore) is not tuple:
			raise TypeError("native geometry repair requires an exact selection tuple")
		if type(target_spacing_points) not in (int, float):
			raise TypeError("native geometry repair spacing must be an exact number")
		import ferrum_chem
		if type(kind) is not ferrum_chem.DocumentGeometryRepairKindV1:
			raise TypeError("native geometry repair requires an exact Ferrum kind")
...
class FerrumNativeGeometryRepairWindowMixin:
	"""Install native repair actions that never inspect or mutate CDML."""
	...
	def _build_geometry_repair_actions(self) -> None:
		...
		self._clean_geometry_action.setToolTip(self.tr(
			"Regenerate selected molecules through native chemistry at an explicit spacing",
		))
	...
	def _on_clean_geometry(self, _checked: bool = False) -> None:
		"""Prepare an atomic native regeneration batch without blocking the UI."""
		...
		try:
			...
			if self._active_native_tab() is not tab:
				raise _tab_error("active native document changed while spacing was open")
			...
			self.statusBar().showMessage(
				self.tr("Cleaning molecule geometry through native chemistry..."), 0,
			)
			self._refresh_actions()
			worker.start()
		except Exception as error:
			self._show_native_file_warning("Clean Geometry Failed", str(error))
			self._refresh_actions()
	...
	def _repair_with_explicit_spacing(
		...
		try:
			...
			tab.repair_geometry_at_revision(
				...
				getattr(ferrum_chem.DocumentGeometryRepairKindV1, kind_name), spacing,
			)
		except Exception as error:
			self._show_native_file_warning(warning_title, str(error))
		finally:
			self._refresh_actions()
	...
	def _on_straighten_bonds(self, _checked: bool = False) -> None:
		...
		try:
			...
			tab.repair_geometry_at_revision(
				revision, molecule_ids, restore,
				ferrum_chem.DocumentGeometryRepairKindV1.straighten_bonds,
				_UNUSED_STRAIGHTEN_SPACING_POINTS,
			)
		except Exception as error:
			self._show_native_file_warning("Terminal-Bond Repair Failed", str(error))
		finally:
			self._refresh_actions()
...
def _tab_error(message: str) -> RuntimeError:
	"""Create the native tab's public error without an import cycle."""
	from ferrum_qt.native.ferrum_native_document_tab import FerrumNativeDocumentTabError

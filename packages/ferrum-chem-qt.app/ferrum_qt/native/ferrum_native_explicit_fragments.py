"""Ordinary native Qt client for Rust-owned explicit fragment annotations."""
...
#============================================
class _CreateExplicitFragmentDialog(PySide6.QtWidgets.QDialog):
...
#============================================
class _ExplicitFragmentViewDialog(PySide6.QtWidgets.QDialog):
...
class FerrumNativeExplicitFragmentsWindowMixin:
	...
	def _on_create_explicit_fragment(self, _checked: bool = False) -> bool:
		...
		while True:
			...
			except ferrum_chem.DocumentExplicitFragmentError:
				if not self._explicit_fragment_capture_is_current(capture):
					dialog.reject()
					self._fragment_changed_before_create()
					return False
	...
	def _on_view_explicit_fragments(self, _checked: bool = False) -> bool:
		...
		try:
			observation = ferrum_chem.inspect_document_explicit_fragments_v1(
				tab.current_document_observation(), snapshot.revision, snapshot.digest,
			)
		except ferrum_chem.DocumentExplicitFragmentError:
			self._show_native_file_warning("Fragments Unavailable", self.tr(

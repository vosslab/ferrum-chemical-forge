"""File menu actions for Ferrum-Qt."""
...
CDML_FILTER = "Ferrum CDML (*.cdml);;All Files (*)"
CHEMISTRY_FILTER = ferrum_qt.io.import_capabilities.chemistry_file_filter()

# map file extensions to OASA codec names for non-CDML formats
_EXTENSION_TO_CODEC = {
	extension: capability.codec_name
	for capability in ferrum_qt.io.import_capabilities.worker_import_capabilities()
...
class _ImportResultRelay(PySide6.QtCore.QObject):
	...
	def on_result(self, result: object) -> None:
		...
		if not self._request_is_current():
			return
		if type(result) is ferrum_qt.bridge.worker.PreparedCompleteCDML:
...
def import_capability(
		main_window: object,
		capability: ferrum_qt.io.import_capabilities.ImportCapability,
		) -> None:
	...
	file_path, _selected_filter = PySide6.QtWidgets.QFileDialog.getOpenFileName(
		...
		"",
		ferrum_qt.io.import_capabilities.capability_file_filter(capability),
	)
	if file_path:
		open_file_path(main_window, file_path)
...
def _load_with_worker(
	...
	"""
	worker = ferrum_qt.bridge.worker.FileImportWorker(codec_name, file_path)
...
def _add_molecules_to_scene(
	...
	projections = ferrum_qt.canvas.molecule_projection.build_molecule_projections(
		molecules,
	)

	if document is None:
		ferrum_qt.canvas.molecule_projection.install_molecule_projections(
			scene, projections,
		)
		return
	...
	if undoable:
		...
		for mol_model, graphics_items in projections:
			document.undo_stack.push(
				ferrum_qt.undo.commands.AddMoleculeCommand(
					document,
					scene,
					mol_model,
					graphics_items,
...
def _paper_properties_request(
	...
	return ferrum_qt.models.document_session.build_paper_properties_request(
...
def register_file_actions(registry: object, app: object) -> None:
	"""Register all File menu actions for Ferrum-Qt.

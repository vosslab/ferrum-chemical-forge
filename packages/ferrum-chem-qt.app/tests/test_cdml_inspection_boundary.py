...
def test_direct_ftext_text_keeps_direct_text_and_cdata_in_authored_order() -> None:
	...
	text = ferrum_qt.io.cdml_inspection.direct_ftext_text(
...
def test_direct_ftext_text_does_not_flatten_nested_markup() -> None:
	...
	text = ferrum_qt.io.cdml_inspection.direct_ftext_text("before<b>nested</b>after")
...
def test_direct_ftext_text_reports_malformed_compatibility_content() -> None:
	...
	text = ferrum_qt.io.cdml_inspection.direct_ftext_text("before<broken>")
...
def test_root_id_preserves_exact_nonempty_compatibility_identifier() -> None:
	...
	identifier = ferrum_qt.io.cdml_inspection.root_id('<fragment id="legacy:one"/>')
...
def test_root_id_ignores_missing_empty_and_malformed_compatibility_content() -> None:
	...
	identifiers = (
		ferrum_qt.io.cdml_inspection.root_id("<fragment/>"),
		ferrum_qt.io.cdml_inspection.root_id('<fragment id=""/>'),
		ferrum_qt.io.cdml_inspection.root_id("<fragment>"),
	)
...
def test_root_id_does_not_reserve_nested_or_namespaced_lookalike_ids() -> None:
	...
	identifiers = (
		ferrum_qt.io.cdml_inspection.root_id('<fragment><child id="nested"/></fragment>'),
		ferrum_qt.io.cdml_inspection.root_id(
...
def test_document_id_planning_reserves_valid_retained_id_and_ignores_malformed_xml(
	...
	molecule = ferrum_qt.models.molecule_model.MoleculeModel(document)
...
def test_plain_qt_consumers_keep_xml_implementation_inside_cdml_io_boundary() -> None:
	...
	imports = (
		_implementation_imports(ferrum_qt.actions.property_editing)
		| _implementation_imports(ferrum_qt.models.document)
	)

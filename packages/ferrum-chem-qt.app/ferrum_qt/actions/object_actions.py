"""Object menu action registrations for Ferrum-Qt."""
...
import ferrum_qt.actions.top_level_transform_actions
...
def _submit_presentation_stack_reorder(app: object, mode: str) -> None:
	...
	if session is None or not session.can_commit_persistent_action:
		...
		return
	root_ids = ferrum_qt.canvas.document_projection.selected_presentation_stack_root_ids(
	...
	if _active_presentation_stack_session(app) is not session:
		...
		return
	request = ferrum_qt.models.document_session.build_presentation_stack_request(
...
def _selection_bounds(objects: list) -> tuple[float, float, float, float] | None:
	...
	bounds = ferrum_qt.geometry.union_bounds([
		ferrum_qt.geometry.top_level_bounds(object_model)
...
def _push_affine_transform(
	...
	for object_model in objects:
		if isinstance(
			object_model, ferrum_qt.models.molecule_model.MoleculeModel,
			):
			for atom_model in object_model.atoms:
				before = (atom_model.x, atom_model.y)
				after = ferrum_qt.geometry.transform_point(
			...
			after_points = [
				(*ferrum_qt.geometry.transform_point(
					(x, y), origin, scale_x, scale_y,
				), z)
				for x, y, z in before_points
			]
			after_bounds = ferrum_qt.geometry.transform_bounds(
	...
	document.undo_stack.push(
		ferrum_qt.undo.commands.TransformGeometryCommand(
...
def _active_legacy_scale_session(app: object) -> object | None:
	...
	session = ferrum_qt.actions.top_level_transform_actions.active_transform_session(app)
	if (
		session is None
...
def _handle_scale_locally(app: object, session: object) -> None:
	...
	# show scale dialog
	result = ferrum_qt.dialogs.scale_dialog.ScaleDialog.get_scale_factors(
		app
	)
	if result is None:
		return
...
def handle_vertical_mirror(app: object) -> None:
	...
	if session is not None and session.legacy_isolated:
		...
		return
	ferrum_qt.actions.top_level_transform_actions.submit_backend_transform(
		app, "mirror-vertical",
	)
...
def handle_scale(app: object) -> None:
	...
	if session is not None:
		...
		return
	ferrum_qt.actions.top_level_transform_actions.submit_scale_transform(
		app, ferrum_qt.dialogs.scale_dialog.ScaleDialog.get_scale_factors,
	)
...
def handle_configure(app: object) -> None:
	...
	plus_capture = ferrum_qt.actions.property_editing.capture_selected_plus_properties(app)
	if plus_capture is not None:
		...
		return
	if ferrum_qt.actions.property_editing.has_single_selected_plus(app):
		return
	...
	wavy_capture = ferrum_qt.actions.property_editing.capture_selected_wavy_properties(app)
...
def register_object_actions(registry: object, app: object) -> None:
	...
	Args:
		...
		app: The main Ferrum-Qt application object providing handler methods.
	"""

"""Ordinary native chooser and one-shot placement for closed D-glucose Haworth recipes."""
...
import ferrum_qt.native.ferrum_native_haworth
...
class FerrumNativeHaworthToolMixin:
	"""Own only the native client lifecycle; Rust owns recipe and durable drawing facts."""
	...
	def _place_haworth(self, intent: _HaworthIntent, event: PySide6.QtGui.QMouseEvent) -> None:
		if self._haworth_intent is not intent or intent.tab.current_snapshot.revision != intent.revision or intent.tab.current_snapshot.digest != intent.digest:
			self._cancel_haworth_intent(); self._show_native_file_warning("Haworth Insertion Changed", "The document changed; choose the Haworth drawing again."); return
		...
		try:
			prepared = ferrum_qt.native.ferrum_native_haworth.prepare_recipe(intent.tab, intent.recipe, anchor)
			preview = ferrum_qt.native.ferrum_native_haworth.create_preview(intent.tab, prepared)
			...
			self._cancel_haworth_intent(); self._show_native_file_warning("Native Haworth Ring Error", str(exc)); return
		self._cancel_haworth_intent()

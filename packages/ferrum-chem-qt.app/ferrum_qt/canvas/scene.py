...
DEFAULT_GRID_SPACING_PT = ferrum_qt.config.geometry_units.DEFAULT_BOND_LENGTH_PT
GRID_Z_VALUE = -100
...
class ChemScene(PySide6.QtWidgets.QGraphicsScene):
	...
	shared YAML theme files in the package resource directory.
	...
	def _build_paper(self) -> None:
		...
		paper_color = ferrum_qt.themes.theme_loader.get_paper_color(self._theme_name)
		outline_color = ferrum_qt.themes.theme_loader.get_paper_outline(self._theme_name)
	...
	def _build_grid(self, spacing: float | None = None) -> None:
		...
		grid_colors = ferrum_qt.themes.theme_loader.get_grid_colors(self._theme_name)
	...
	def apply_theme(self, theme_name: str) -> None:
		...
		paper_color = ferrum_qt.themes.theme_loader.get_paper_color(theme_name)
		outline_color = ferrum_qt.themes.theme_loader.get_paper_outline(theme_name)
	...
	def _recolor_grid(self, theme_name: str) -> None:
		...
		if self._grid_overlay is None:
			return
		grid_colors = ferrum_qt.themes.theme_loader.get_grid_colors(theme_name)
	...
	def snap_to_grid(self, x: float, y: float) -> tuple:
		...
		"""
		snapped = ferrum_qt.bridge.display_geometry.snap_to_hex_grid(

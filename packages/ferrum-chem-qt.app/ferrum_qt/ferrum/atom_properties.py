"""Typed AtomDialog adaptation for the Ferrum editor."""
...
def dialog_model_from_projection(atom: object) -> FerrumNativeAtomDialogModel:
	...
	if type(atom) is not engine.AtomProjectionV1:
		raise TypeError("Ferrum atom properties require an exact Ferrum atom projection")
	...
	font_size, line_color = _dialog_font_values(atom.label_font, engine)
...
def property_changes_from_dialog(changes: tuple[tuple[str, object], ...]) -> tuple[object, ...]:
	...
	if type(changes) is not tuple:
		raise TypeError("Ferrum atom property changes must be an exact tuple")
	converted = []
	for change in changes:
		if type(change) is not tuple or len(change) != 2:
			raise TypeError("Ferrum atom property changes must be exact field/value pairs")
		field, value = change
...
#============================================
def _dialog_font_values(font: object, extension: object) -> tuple[int, str]:
	"""Return the exact label-font values that the integer visual control can show."""
	if font is None:
		return 12, "#000000"
	if type(font) is not extension.FontFactsV1:
...
def _property_change(field: object, value: object, extension: object) -> object:
	...
	if field == "element" and type(value) is str:
		return extension.DocumentAtomPropertyChangeV1.element(value)
	if field == "charge" and type(value) is int:
		return extension.DocumentAtomPropertyChangeV1.formal_charge(value)
	if field == "valency" and type(value) is int:
		valence = None if value == 0 else value
		return extension.DocumentAtomPropertyChangeV1.valence(valence)
	if field == "isotope" and (value is None or type(value) is int):
		return extension.DocumentAtomPropertyChangeV1.isotope(value)
	if field == "multiplicity" and type(value) is int:
		return extension.DocumentAtomPropertyChangeV1.multiplicity(value)
	if field == "show" and type(value) is bool:
		return extension.DocumentAtomPropertyChangeV1.show(value)
	if field == "show_hydrogens" and type(value) is bool:
		return extension.DocumentAtomPropertyChangeV1.show_hydrogens(value)
	if field == "font_size" and type(value) is int:
		return extension.DocumentAtomPropertyChangeV1.font_size(float(value))
	if field == "line_color" and type(value) is str:
		return extension.DocumentAtomPropertyChangeV1.label_color(value)
	raise ValueError("Ferrum AtomDialog supplied an unsupported property change")

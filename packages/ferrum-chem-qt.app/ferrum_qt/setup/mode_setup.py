...
import ferrum_qt.modes.text_mode
import ferrum_qt.modes.mark_mode
import ferrum_qt.modes.atom_mode
import ferrum_qt.modes.rotate_mode
import ferrum_qt.modes.bondalign_mode
import ferrum_qt.modes.bracket_mode
import ferrum_qt.modes.vector_mode
import ferrum_qt.modes.repair_mode
import ferrum_qt.modes.plus_mode
import ferrum_qt.modes.misc_mode
import ferrum_qt.modes.file_actions_mode
import ferrum_qt.resource_paths

# Modes are package-owned so installed Qt wheels do not need the source tree.
_MODES_YAML_PATH = ferrum_qt.resource_paths.get_resource_path("modes.yaml")
...
def setup_modes(
	...
	"""
	mode_manager = ferrum_qt.modes.mode_manager.ModeManager(
	...
	mode_manager.register_mode(
		"file_actions",
		ferrum_qt.modes.file_actions_mode.FileActionsMode(
	...
	mode_manager.register_mode(
		"template",
		ferrum_qt.modes.template_mode.TemplateMode(view, template_names=template_names),
	)
	mode_manager.register_mode(
		"biotemplate",
		ferrum_qt.modes.biotemplate_mode.BioTemplateMode(
	...
	mode_manager.register_mode(
		"arrow",
		ferrum_qt.modes.arrow_mode.ArrowMode(view),
	)
	mode_manager.register_mode(
		"text",
		ferrum_qt.modes.text_mode.TextMode(view),
	)
	mode_manager.register_mode(
		"rotate",
		ferrum_qt.modes.rotate_mode.RotateMode(view),
	)
	mode_manager.register_mode(
		"mark",
		ferrum_qt.modes.mark_mode.MarkMode(view),
	)
	mode_manager.register_mode(
		"atom",
		ferrum_qt.modes.atom_mode.AtomMode(view),
	)
	mode_manager.register_mode(
		"bondalign",
		ferrum_qt.modes.bondalign_mode.BondAlignMode(view),
	)
	mode_manager.register_mode(
		"bracket",
		ferrum_qt.modes.bracket_mode.BracketMode(view),
	)
	mode_manager.register_mode(
		"vector",
		ferrum_qt.modes.vector_mode.VectorMode(view),
	)
	mode_manager.register_mode(
		"repair",
		ferrum_qt.modes.repair_mode.RepairMode(view),
	)
	mode_manager.register_mode(
		"plus",
		ferrum_qt.modes.plus_mode.PlusMode(view),
	)
	mode_manager.register_mode(
		"misc",
		ferrum_qt.modes.misc_mode.MiscMode(view),
	)
...
def _inject_submodes_from_yaml(mode_manager: object) -> None:
	...
	for mode_name in mode_manager.mode_names():
		...
		parsed = ferrum_qt.modes.config.load_submodes_from_yaml(cfg)

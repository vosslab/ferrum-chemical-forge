...
def dialog_model_from_projection(plus: object) -> FerrumNativePlusDialogModel:
	...
	import ferrum_chem
	if type(plus) is not ferrum_chem.PlusProjectionV1:
		raise TypeError("native Plus properties require an exact Ferrum Plus projection")
	size = plus.font.size
...
def property_changes_from_dialog(
	...
	if type(changes) is not tuple:
		raise TypeError("native Plus property changes must be an exact tuple")
	converted = []
	for change in changes:
		if type(change) is not tuple or len(change) != 2:
			raise TypeError("native Plus property changes must be exact field/value pairs")
		field, value = change
		if field == "font_size" and type(value) is int:
			converted.append(ferrum_chem.DocumentPlusPropertyChangeV1.font_size(value))
		elif field == "color" and type(value) is str:
			converted.append(ferrum_chem.DocumentPlusPropertyChangeV1.color(value))
		else:
			raise ValueError("native PlusDialog supplied an unsupported property change")
...
def install_plus_properties_action(window: object,
	...
	"""Install one native Plus action without adding dialog logic to the host."""
	action = PySide6.QtGui.QAction(window.tr("Edit Plus Properties"), window)
	action.setToolTip(window.tr(
		"Edit one selected durable Plus through one Rust-native operation",
	))
...
def _on_edit_plus_properties(window: object) -> None:
	...
	try:
		...
		window._show_native_file_warning("Native Plus Properties Unavailable", str(exc))
		return
	...
	try:
		...
		window._refresh_actions()
		window._show_native_file_warning("Native Plus Properties Error", str(exc))
		return
	window.statusBar().showMessage(window.tr("Updated one Rust-native Plus."), 5000)
	window._refresh_actions()

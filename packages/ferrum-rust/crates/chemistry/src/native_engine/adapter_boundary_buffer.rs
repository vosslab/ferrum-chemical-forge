...
use super::{AdapterError, BufferFreeFn};
...
pub(crate) fn checked_length(length: u64) -> Result<usize, AdapterError> {
    let maximum = u64::try_from(FERRUM_CHEM_MAX_RESPONSE_BYTES)
        .expect("adapter response limit fits the foreign ABI length");
    if length > maximum {
        return Err(AdapterError::ResponseTooLarge { length, maximum });
    }

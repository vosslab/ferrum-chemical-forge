"""Private local-source identity, provenance, and bootstrap state for native Open."""
...
class FerrumNativeLocalCdmlOriginTabMixin:
	...
	def _adopt_local_cdml_origin_token(self, token: object) -> None:
		...
		self._require_live()
		if token is None:
			raise TypeError("native local CDML origin requires a Rust identity token")
		if self._local_cdml_origin_token is not None:
			raise ValueError("native tab already has a local CDML origin identity")
	...
	def _adopt_local_document_origin(
		...
		if source_kind not in {"cdml", "decoded_cdsvg"}:
			raise ValueError("native document origin has an unknown source kind")
		if self._local_document_source_path is not None:
			raise ValueError("native tab already has local document source provenance")

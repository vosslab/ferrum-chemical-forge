//! Strict recovery of Ferrum-owned SDF metadata from one direct molecule.

use thiserror::Error;
use xot::{Node, Xot};

use super::{CDML_NAMESPACE, SDF_IMPORT_NAMESPACE_V1, TypedClass, TypedDocument};

/// One exact ordered SDF property recovered from persisted CDML.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SdfPropertyMetadataV1 {
    name: String,
    value: String,
}

impl SdfPropertyMetadataV1 {
...
/// Exact imported SDF record metadata retained below one molecule.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SdfRecordMetadataV1 {
    title: String,
    properties: Vec<SdfPropertyMetadataV1>,
}

impl SdfRecordMetadataV1 {
    ...
    #[must_use]
    pub fn properties(&self) -> &[SdfPropertyMetadataV1] {
        &self.properties
    }

    /// Consume this record into its exact owned title and property sequence.
    #[must_use]
    pub fn into_parts(self) -> (String, Vec<SdfPropertyMetadataV1>) {
        (self.title, self.properties)
    }
}

/// Recover one direct molecule's optional Ferrum SDF import metadata.
///
...
/// # Errors
///
/// Returns [`SdfRecordMetadataErrorV1`] when the selector is not one typed
...
pub fn observe_sdf_record_metadata_v1(
    document: &TypedDocument,
    molecule_source_id: &str,
) -> Result<Option<SdfRecordMetadataV1>, SdfRecordMetadataErrorV1> {
    ...
    if typed_match_count != 1 {
        return Err(SdfRecordMetadataErrorV1::UnknownDirectMolecule);
    }
    ...
    let root = tree
        ...
        .map_err(|_| SdfRecordMetadataErrorV1::MalformedRecord)?;
    ...
    for child in tree.children(molecule) {
        if !is_element(tree, child, "sdf-record", SDF_IMPORT_NAMESPACE_V1) {
            continue;
        }
        if metadata_node.replace(child).is_some() {
            return Err(SdfRecordMetadataErrorV1::DuplicateMetadata);
        }
    }
    metadata_node
...
fn decode_record(tree: &Xot, node: Node) -> Result<SdfRecordMetadataV1, SdfRecordMetadataErrorV1> {
    ...
    for (name, value) in tree.attributes(node).iter() {
        ...
        match (namespace, local) {
            ...
            _ => return Err(SdfRecordMetadataErrorV1::MalformedRecord),
        }
    }
    if encoding != Some("utf8-hex-v1") {
        return Err(SdfRecordMetadataErrorV1::UnsupportedEncoding);
    }
    let title = decode_utf8_hex(title.ok_or(SdfRecordMetadataErrorV1::MalformedRecord)?)?;
    if title.contains(['\0', '\r', '\n']) {
        return Err(SdfRecordMetadataErrorV1::InvalidTitle);
    }

    let mut properties = Vec::new();
    for child in tree.children(node) {
        if !is_element(tree, child, "property", SDF_IMPORT_NAMESPACE_V1) {
            return Err(SdfRecordMetadataErrorV1::MalformedRecord);
        }
        properties
            .try_reserve(1)
            .map_err(|_| SdfRecordMetadataErrorV1::ResourceAllocation)?;
        properties.push(decode_property(tree, child)?);
    }
    Ok(SdfRecordMetadataV1 { title, properties })
}

fn decode_property(
    tree: &Xot,
    node: Node,
) -> Result<SdfPropertyMetadataV1, SdfRecordMetadataErrorV1> {
    if tree.children(node).next().is_some() {
        return Err(SdfRecordMetadataErrorV1::MalformedProperty);
    }
    let mut name = None;
    let mut value = None;
    for (attribute, text) in tree.attributes(node).iter() {
        ...
        match (namespace, local) {
            ...
            _ => return Err(SdfRecordMetadataErrorV1::MalformedProperty),
        }
    }
    let name = decode_utf8_hex(name.ok_or(SdfRecordMetadataErrorV1::MalformedProperty)?)?;
    let value = decode_utf8_hex(value.ok_or(SdfRecordMetadataErrorV1::MalformedProperty)?)?;
    if name.is_empty() || name.contains(['\0', '\r', '\n', '<', '>']) {
        return Err(SdfRecordMetadataErrorV1::InvalidPropertyName);
    }
    ...
    {
        return Err(SdfRecordMetadataErrorV1::InvalidPropertyValue);
    }
    Ok(SdfPropertyMetadataV1 { name, value })
}

fn decode_utf8_hex(value: &str) -> Result<String, SdfRecordMetadataErrorV1> {
    if !value.len().is_multiple_of(2)
        || !value
            .bytes()
            .all(|byte| byte.is_ascii_digit() || matches!(byte, b'a'..=b'f'))
    {
        return Err(SdfRecordMetadataErrorV1::InvalidHex);
    }
    ...
    String::from_utf8(bytes).map_err(|_| SdfRecordMetadataErrorV1::InvalidUtf8)
}
...
/// Rejection of a direct-root selector or authoritative persisted SDF metadata.
#[derive(Clone, Debug, Eq, Error, PartialEq)]
pub enum SdfRecordMetadataErrorV1 {
    /// The source ID does not identify exactly one typed direct molecule.
    #[error("SDF metadata selector is not one typed direct molecule")]
    UnknownDirectMolecule,
    /// One molecule carries more than one authoritative metadata record.
    #[error("direct molecule carries duplicate Ferrum SDF metadata records")]
    DuplicateMetadata,
    /// The authoritative metadata record has unsupported attributes or children.
    #[error("Ferrum SDF metadata record structure is malformed")]
    MalformedRecord,
    /// The authoritative property has unsupported attributes or children.
    #[error("Ferrum SDF metadata property structure is malformed")]
    MalformedProperty,
    /// The retained encoding is absent or not the closed V1 spelling.
    #[error("Ferrum SDF metadata encoding is not utf8-hex-v1")]
    UnsupportedEncoding,
    /// One retained field is not lowercase even-length hexadecimal text.
    #[error("Ferrum SDF metadata contains invalid hexadecimal text")]
    InvalidHex,
    /// One decoded field is not UTF-8.
    #[error("Ferrum SDF metadata contains invalid UTF-8")]
    InvalidUtf8,
    /// The decoded title violates the persisted single-line grammar.
    #[error("Ferrum SDF metadata title is invalid")]
    InvalidTitle,
    /// A decoded property name violates the persisted field grammar.
    #[error("Ferrum SDF metadata property name is invalid")]
    InvalidPropertyName,
    /// A decoded property value violates the persisted field grammar.
    #[error("Ferrum SDF metadata property value is invalid")]
    InvalidPropertyValue,
    /// Exact decoded metadata storage could not be reserved.
    #[error("Ferrum SDF metadata storage could not be reserved")]
    ResourceAllocation,
}

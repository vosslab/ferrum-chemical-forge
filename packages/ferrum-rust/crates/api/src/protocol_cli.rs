...
use crate::streams::is_standard_stream;
use crate::{
    OperationProtocolInputErrorV1, execute_operation_v1, generated_operation_protocol_schema_v1,
};
...
pub(crate) fn run_protocol(
    ...
    let envelope = execute_operation_v1(&request).map_err(protocol_input_error)?;
...
fn read_request_bytes(reader: &mut dyn Read, input: String) -> Result<String, ProtocolCliError> {
    let limit = crate::OPERATION_PROTOCOL_REQUEST_UTF8_BYTES_V1;

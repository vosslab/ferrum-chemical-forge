...
pub(crate) fn prepare_molecule_coordinates_v1(
    ...
    let molecule_id = DocumentObjectIdV1::parse(molecule_id)
        .map_err(|error| super::binding::operation_validation_error(py, error.to_string()))?;
    let observation = observation.observation().clone();
    let library_path = super::chemistry_binding::packaged_library_path(py, MOLECULE_OPERATION)?;
    ...
    match result {
        ...
        Err(NativePreparationFailure::Load(error)) => Err(
            super::chemistry_binding::map_load_error(py, MOLECULE_OPERATION, &library_path, error)?,
        ),
...
pub(crate) fn prepare_clean_geometry_v1(
    ...
    let observation = observation.observation().clone();
    let library_path = super::chemistry_binding::packaged_library_path(py, CLEAN_OPERATION)?;
    let worker_path = library_path.clone();
    let result = py.detach(move || {
        let engine =
            NativeChemEngine::load(&worker_path).map_err(NativePreparationFailure::Load)?;
    ...
    match result {
        ...
        Err(NativePreparationFailure::Load(error)) => Err(
            super::chemistry_binding::map_load_error(py, CLEAN_OPERATION, &library_path, error)?,
        ),
...
fn map_clean_build_error(
    ...
    match error {
        ...
        | CleanGeometryBuildError::DuplicateMolecule => Err(
            super::binding::operation_validation_error(py, error.to_string()),
        ),
        CleanGeometryBuildError::Target { source, .. } => {
            map_build_error(py, CLEAN_OPERATION, library_path, source)
        }
        CleanGeometryBuildError::Geometry(error) => Ok(super::geometry_binding::geometry_error(
            py,
            error.to_string(),
        )),
...
fn exact_positive_finite(py: Python<'_>, value: &Bound<'_, PyAny>) -> PyResult<f64> {
    ...
    let value = value.extract::<f64>().map_err(|_| {
        super::binding::operation_validation_error(
            py,

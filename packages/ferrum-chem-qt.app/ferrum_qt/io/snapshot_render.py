...
class _SnapshotProjection:
	...
	prepared: ferrum_qt.io.cdml_document_io.PreparedProjection
	items: list[object]
	...
	def dispose(self) -> None:
		...
		try:
			...
			record = ferrum_qt.canvas.graphics_retirement.temporary_scene_retirement_reaper.retire(
		...
		try:
			ferrum_qt.io.cdml_document_io.dispose_prepared_projection(self.prepared)
		except Exception as exc:
			errors.append(exc)
		if errors:
...
def _render_bounds(
	...
	if request.scope == "selection":
		return ferrum_qt.io.render_plan.build_render_plan(
	...
	return ferrum_qt.io.render_plan.RenderPlan(
...
#============================================
def _render_bytes(scene: object, plan: ferrum_qt.io.render_plan.RenderPlan,

//! Operation envelopes, operation dispatch types, and protocol-wide limits.
...
pub(in crate::protocol) const MAX_ARTIFACT_BYTES_V1: usize = smallest_artifact_limit(
    LOCAL_SVG_COMPLETED_BYTES_V1,
    LOCAL_PDF_COMPLETED_BYTES_V1,
    LOCAL_PNG_ENCODED_BYTES_V1,
);
pub(in crate::protocol) const MAX_ARTIFACT_BASE64_BYTES_V1: usize =
    base64_encoded_len(MAX_ARTIFACT_BYTES_V1)
...
pub enum OperationProtocolOperationV1 {
    ...
    /// Observe one selected atom's oxidation state from a fenced document snapshot.
    #[serde(rename = "document.atom.oxidation.observe.v1")]
    DocumentAtomOxidationObserve(DocumentAtomOxidationObserveRequestV1),
...
pub enum OperationProtocolOutcomeV1 {
    ...
    /// One typed selected-atom oxidation observation.
    #[serde(rename = "document.atom.oxidation.observe.v1")]
    DocumentAtomOxidationObserve {
        observation: DocumentAtomOxidationObservationV1,
    },
    /// One renderer-safe explicit-hydrogen materialization result.
    #[serde(rename = "document.molecule.hydrogen.materialize.v1")]
...
pub(in crate::protocol) const fn base64_encoded_len(byte_len: usize) -> Option<usize> {

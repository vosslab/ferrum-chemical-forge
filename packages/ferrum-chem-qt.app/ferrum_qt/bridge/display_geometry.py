...
def hex_grid_points(
	...
	"""
	return engine.hex_grid_points_v1(x_min, y_min, x_max, y_max, spacing)
...
def hex_grid_edges(
	...
	return engine.hex_grid_edges_v1(x_min, y_min, x_max, y_max, spacing)
...
def snap_to_hex_grid(x: float, y: float, spacing: float) -> tuple[float, float]:
	...
	Raises:
		ValueError: If a coordinate or spacing is non-finite, or spacing is not
			positive.
	"""
	return engine.snap_to_hex_grid_v1(x, y, spacing)
...
def normalize_hex_grid_spacing(value: float) -> float:
	...
	"""
	return engine.normalize_hex_grid_spacing_v1(value)

"""Current projection receipt for native read-only property clients."""
	...
	def observe_properties(self) -> FerrumNativePropertyObservation:
		...
		if self._document_observation is None:
			raise native_document_tab_errors.FerrumNativeDocumentTabError(
				"native tab has no installed document projection",
			)
		...
		if (
			...
			raise native_document_tab_errors.FerrumNativeDocumentTabError(
				"native property observation provenance is inconsistent",
			)

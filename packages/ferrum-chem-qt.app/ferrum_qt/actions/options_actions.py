"""Options menu action registrations for Ferrum-Qt."""
...
# preference is a Ferrum application setting, not a serialized CDML property.
_LOGGING_LEVELS = {
...
def apply_saved_logging_level(
		prefs: ferrum_qt.config.preferences.Preferences,
		) -> str:
	"""Apply the persisted Ferrum logging level and return its label."""
...
def _show_logging_dialog(app: object) -> None:
	...
	Args:
		app: The main Ferrum-Qt application window.
	"""
	prefs = app._prefs
	...
	prefs.set_value(
		ferrum_qt.config.preferences.Preferences.KEY_LOGGING_LEVEL, chosen,
	)
	apply_saved_logging_level(prefs)
	app.statusBar().showMessage(
		f"Logging level set to {chosen} for this and future Ferrum launches", 5000
	)
...
def register_options_actions(registry: object, app: object) -> None:
	...
	registry.register(MenuAction(
		...
		help_key='Set Ferrum logging verbosity now and for future launches',
		accelerator=None,

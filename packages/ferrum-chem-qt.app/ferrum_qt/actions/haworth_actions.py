"""Editable Haworth sugar insertion actions for Ferrum-Qt."""
...
def _capture_haworth_geometry(target: object) -> tuple[float, tuple[float, float]]:
	...
	return ferrum_qt.bridge.insertion_placement.capture_insertion_placement(target)
...
class HaworthInsertionDelivery:
	...
	def deliver(
		...
		proposal = ferrum_qt.bridge.chemistry_preparation.molecule_insertion_proposal(prepared)
		if proposal is None:
...
def _start_verified_sucrose_insert(app: object) -> None:
	...
	worker = ferrum_qt.bridge.worker.OasaWorker(
		ferrum_qt.bridge.chemistry_preparation.prepare_verified_sucrose_insertion,
		expected_revision,
		token_stem,
		bond_length_pt,
		insertion_anchor,

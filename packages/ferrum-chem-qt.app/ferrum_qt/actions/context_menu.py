...
import ferrum_qt.canvas.scene_queries
...
def show_context_menu(view: object, scene_pos: object, screen_pos: object) -> None:
	...
	if isinstance(target_item, ferrum_qt.canvas.items.atom_item.AtomItem):
		menu = _atom_context_menu(target_item, view)
	elif isinstance(target_item, ferrum_qt.canvas.items.bond_item.BondItem):
...
def _atom_context_menu(atom_item: object, view: object) -> PySide6.QtWidgets.QMenu:
	...
	molecule = ferrum_qt.canvas.scene_queries.find_molecule_for_atom(view, atom_model)
...
def _delete_atom_local(view: object, atom_item: object) -> None:
	...
	if scene is None:
		return
	undo_stack = ferrum_qt.canvas.scene_queries.find_undo_stack(view)
	atom_model = atom_item.atom_model
	mol_model = ferrum_qt.canvas.scene_queries.find_molecule_for_atom(view, atom_model)
	if mol_model is None or undo_stack is None:
		return
	connected_bonds = ferrum_qt.canvas.scene_queries.find_connected_bond_items(scene, atom_model)
...
def _structure_delete_callback(view: object, item: object) -> object:
	...
	if _active_document_session(view) is not session:
		from ferrum_qt.models import document_session
...
def _resolve_local_atom(
	...
	if not _is_valid_qobject(atom_model):
		return None
	if ferrum_qt.canvas.scene_queries.find_molecule_for_atom(view, atom_model) is None:
		return None
	return atom_model
...
def _resolve_local_bond(
	...
	if not _is_valid_qobject(bond_model):
		return None
	if ferrum_qt.canvas.scene_queries.find_molecule_for_bond(view, bond_model) is None:
		return None
	return bond_model
...
def _atom_properties_callback(
	...
	if kind == "synchronized":
		def invoke_synchronized() -> None:
			...
			ferrum_qt.actions.property_editing.edit_atom_properties(
				atom_model, current_view,
				ferrum_qt.canvas.scene_queries.find_undo_stack(current_view),
			)
	...
	def invoke_local() -> None:
		...
		if atom_model is None:
			return
		ferrum_qt.actions.property_editing.edit_atom_properties(
			atom_model, view, ferrum_qt.canvas.scene_queries.find_undo_stack(view),
		)
	return invoke_local
...
def _bond_context_menu(bond_item: object, view: object) -> PySide6.QtWidgets.QMenu:
	...
	molecule = ferrum_qt.canvas.scene_queries.find_molecule_for_bond(view, bond_model)
	molecule_id = getattr(molecule, "mol_id", None)
	...
	for type_char, label in ferrum_qt.bond_presentation.ORDINARY_BOND_TYPE_CHOICES:
	...
	menu._ferrum_submenus = (order_menu, type_menu)
...
def _bond_properties_callback(
	...
	if kind == "synchronized":
		def invoke_synchronized() -> None:
			...
			ferrum_qt.actions.property_editing.edit_bond_properties(
	...
	def invoke_local() -> None:
		...
		if bond_model is None:
			return
		ferrum_qt.actions.property_editing.edit_bond_properties(
			bond_model, view, ferrum_qt.canvas.scene_queries.find_undo_stack(view),
		)
	return invoke_local
...
def _delete_bond_local(view: object, bond_item: object) -> None:
	...
	undo_stack = ferrum_qt.canvas.scene_queries.find_undo_stack(view)
	bond_model = bond_item.bond_model
	mol_model = ferrum_qt.canvas.scene_queries.find_molecule_for_bond(view, bond_model)
	if mol_model is None or undo_stack is None:
		return
...
def _set_bond_type(
	...
	"""
	if (
		bond_type not in dict(ferrum_qt.bond_presentation.ORDINARY_BOND_TYPE_CHOICES)
		or not isinstance(molecule_id, str) or not molecule_id
		or not isinstance(bond_id, str) or not bond_id
	):
		return

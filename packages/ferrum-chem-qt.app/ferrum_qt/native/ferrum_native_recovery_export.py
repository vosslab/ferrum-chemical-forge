...
class FerrumNativeRecoveryExportWindowMixin:
	"""Own only the native Recovery Export action and its provenance fence."""
	...
	def _refresh_recovery_export_action(
		...
		"""Keep recovery reachable for every exact live registered native tab."""
	...
	def _on_native_recovery_export(self) -> bool:
		...
		if not self._recovery_capture_is_current(capture):
			self._show_native_file_warning(
				...
				"The active Rust backend snapshot changed while choosing a destination. "
		...
		if not self._recovery_receipt_matches_capture(publication, capture):
			self._show_native_file_warning(
				"Recovery Export Unavailable",
				"Rust returned publication provenance that does not match the selected "
		...
		if publication.outcome.is_confirmed:
			...
			return True
		self._show_native_file_warning(
			"Recovery Export Durability Unconfirmed",
			"The exact Rust backend snapshot may be present at %s, but directory-entry "
	...
	def _capture_native_recovery_export(self) -> FerrumNativeRecoveryExportCapture | None:
		...
		try:
			...
			self._show_native_file_warning("Recovery Export Unavailable", str(exc))
			return None
	...
	def _normalize_recovery_export_path(self, selected_path: str) -> str | None:
		"""Apply native CDML suffix policy without reserving or adopting the path."""
		path = pathlib.Path(selected_path)
		if not path.suffix:
			path = path.with_suffix(".cdml")
		elif path.suffix.lower() != ".cdml":
			self._show_native_file_warning(
				"Unsupported Recovery Export Format",
				"Ferrum recovery exports must use the .cdml extension.",
			)
			return None
	...
	def _report_native_recovery_export_error(self, absolute_path: str, exc: Exception) -> bool:
		...
		if type(exc) is ferrum_chem.PublicationPossiblyCompletedError:
			title = "Recovery Export Possibly Completed"
			message = (

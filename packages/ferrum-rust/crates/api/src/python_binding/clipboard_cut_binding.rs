...
fn prepare_document_clipboard_cut_v1_binding(
    ...
    let selection = super::clipboard_fragment_binding::parse_clipboard_selection(
        py,
    ...
    let source_digest = super::clipboard_fragment_binding::hex_digest(py, plan.source_digest())
        .map_err(|error| cut_error(py, error.to_string()))?;
    let selected_objects =
        super::clipboard_fragment_binding::object_tuple(py, plan.selected_objects())
            .map_err(|error| cut_error(py, error.to_string()))?;

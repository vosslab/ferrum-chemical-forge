use ferrum_api::{
    ReactionCreateRequestV1, begin_api_reaction_gesture_v1, prepare_api_reaction_gesture_v1,
};
...
fn reaction_capabilities_remain_redacted_across_public_document_render_and_api_surfaces() {
    ...
    let bridge_prepared = prepare_reaction_gesture_v1(&mut bridge_session, &bridge_gesture)
        .expect("bridge capability prepares");
    let bridge_debug = format!("{bridge_prepared:?}");
    assert_redacted(&bridge_debug);
    assert!(bridge_debug.contains("prepared"));
    ...
    let api_prepared = prepare_api_reaction_gesture_v1(&mut api_session, &api_gesture)

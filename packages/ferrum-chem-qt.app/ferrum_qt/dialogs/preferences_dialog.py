...
class PreferencesDialog(PySide6.QtWidgets.QDialog):
	...
	def _create_shortcuts_tab(self) -> PySide6.QtWidgets.QWidget:
		...
		shortcut_timing = PySide6.QtWidgets.QLabel(
			self.tr("Shortcut changes take effect the next time you start Ferrum."),
		)
	...
	def _populate_shortcuts_table(self) -> None:
		...
		if manager is None:
			bindings = dict(ferrum_qt.config.keybindings.DEFAULT_KEYBINDINGS)
		else:
	...
	def _reset_shortcuts(self) -> None:
		...
		manager = getattr(self.parent(), "_keybinding_manager", None)
		if manager is None:
			bindings = ferrum_qt.config.keybindings.DEFAULT_KEYBINDINGS
		else:
	...
	def _apply(self) -> bool:
		...
		if manager is not None:
			try:
			...
			except ferrum_qt.config.keybindings.KeybindingConflictError as exc:
				...
				return False
			except ferrum_qt.config.keybindings.KeybindingRegistrationError as exc:
				PySide6.QtWidgets.QMessageBox.warning(
					self,

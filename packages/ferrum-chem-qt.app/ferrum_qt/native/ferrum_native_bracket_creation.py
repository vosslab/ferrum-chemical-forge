"""Rust-owned bracket-pair creation for the standalone native document tab."""
...
class FerrumNativeBracketCreationMixin:
	...
	def create_round_bracket(self, left: float, top: float,
		...
		import ferrum_chem
		return self._create_bracket_pair(
			ferrum_chem.DocumentBracketStyleV1.round,
	...
	def _create_bracket_pair(self, style: object, root_kind: str,
		...
		if any(type(value) is not float for value in (left, top, right, bottom)):
			raise TypeError("native bracket bounds must be floats")
		import ferrum_chem
		if type(style) is not ferrum_chem.DocumentBracketStyleV1:
			raise TypeError("native bracket style must be an exact Rust bracket style")
		if root_kind not in ("polyline", "round_bracket"):
			raise ValueError("native bracket root kind is not supported")
		revision = self.current_snapshot.revision
		bounds = ferrum_chem.DocumentBracketBoundsV1(left, top, right, bottom)

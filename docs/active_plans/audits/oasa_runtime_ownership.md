...
production OASA declarations are retired. Historical OASA and BKChem references may
remain as provenance or isolated oracle inputs; they are not production runtime owners.

## Current production boundary

No production Ferrum-Qt module imports OASA or dynamically loads the retired host.
The root requirements and Ferrum-Qt project metadata no longer declare OASA. This
is recorded as a retirement result, not maintained as a source-count gate.
...
| Oracle-only OASA inputs | Isolated historical E2E/oracle boundary | Not a production dependency or desktop fallback |
...
| FQ-014 | Rust owns the bounded `ferrum-native-peptide-template-insertion-v1` route for its documented native-17 profile. | System and biomolecule template catalogs, descriptive catalogs, and generic peptide authoring remain pre-production drops; bounded insertion is not OASA parity. |
...
no OASA, Python RDKit, or Tk runtime dependency. Oracle-only use remains outside production and
must stay explicitly isolated. Repository inspection and installed-window walkthroughs may
support that release decision as disposable evidence; they are not permanent source-count gates.

"""Rust-authoritative renderable drawing defaults for native documents."""
...
class FerrumNativeDrawingStandardTabMixin:
	...
	def apply_drawing_standard(self, changes: tuple[object, ...]) -> object:
		...
		if type(changes) is not tuple:
			raise TypeError("native drawing-standard changes require an exact tuple")
		import ferrum_chem
		if any(
				type(change) is not ferrum_chem.DocumentDrawingStandardPropertyChangeV1
				for change in changes
				):
			raise TypeError("native drawing-standard changes require frozen Ferrum values")
		operation = ferrum_chem.DocumentOperationV1.set_drawing_standard(changes)
...
#============================================
class FerrumNativeDrawingStandardDialog(PySide6.QtWidgets.QDialog):
...
def model_from_projection(standard: object | None) -> FerrumNativeDrawingStandardModel:
	...
	import ferrum_chem
	if standard is not None and type(standard) is not ferrum_chem.DrawingStandardV1:
		raise TypeError("native drawing standard requires exact frozen Ferrum facts")
	values = {
		"line_width": 1.0,
		"font_size": 12.0,
		"line_color": "#000000",
...
def changes_from_dialog(changes: tuple[tuple[str, object], ...]) -> tuple[object, ...]:
	...
	if type(changes) is not tuple:
		raise TypeError("native drawing-standard dialog changes must be an exact tuple")
	import ferrum_chem
	change_type = ferrum_chem.DocumentDrawingStandardPropertyChangeV1
	converted: list[object] = []
	for change in changes:
		if type(change) is not tuple or len(change) != 2:
			raise TypeError("native drawing-standard changes must be exact field/value pairs")
		field, value = change
		...
		if factory is None or field not in {
			...
			raise ValueError("native drawing-standard dialog supplied an unsupported field")
...
def refresh_drawing_standard_action(action: PySide6.QtGui.QAction,
	...
	"""Disable editing while the native tab cannot accept one operation."""
...
def _on_edit_drawing_standard(window: object) -> None:
	...
	try:
		...
		window._show_native_file_warning("Native Drawing Defaults Unavailable", str(exc))
		return
	...
	try:
		...
		window._refresh_actions()
		window._show_native_file_warning("Native Drawing Defaults Error", str(exc))
		return
	window.statusBar().showMessage(window.tr("Updated Rust-native drawing defaults."), 5000)
	window._refresh_actions()

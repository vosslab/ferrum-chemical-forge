"""Ferrum catalog, publication, and placement for user templates."""
...
class FerrumNativeUserTemplateTabMixin:
	...
	def insert_user_template(self, prepared: object, x: float, y: float) -> object:
		...
		if type(prepared) is not engine.DocumentUserTemplatePlanV1:
			raise TypeError("Ferrum user-template insertion requires an exact Ferrum plan")
		if type(x) is not float or type(y) is not float:
			raise TypeError("Ferrum user-template anchor coordinates must be floats")
...
class FerrumNativeUserTemplateWindowMixin:
	"""Own the Ferrum template catalog, actions, one-click intent, and save flow."""
	...
	def _on_place_user_template(self, checked: bool) -> None:
		...
		if not entries:
			self._cancel_user_template_placement()
			self._show_edit_refusal(self._unavailable_edit_refusal("Save or add an eligible .cdml template, then refresh the catalog."))
			return
	...
	def start_user_template_placement(self, catalog_key: str) -> bool:
		...
		if type(catalog_key) is not str or not catalog_key:
			raise TypeError("Ferrum user-template placement requires a catalog key")
	...
	def _native_user_template_other_work_busy(self) -> bool:
		"""Return whether another workflow currently owns interaction."""
		return (
	...
	def eventFilter(self, watched: PySide6.QtCore.QObject,
		...
		"""Capture the template placement click before other Ferrum gesture owners."""
	...
	def _complete_user_template_placement(self, event: PySide6.QtGui.QMouseEvent) -> None:
		...
		if not self._user_template_placement_is_current(intent):
			self._cancel_user_template_placement()
			self._show_edit_refusal(self._unavailable_edit_refusal("The document or catalog changed; choose the template again."))
			return
		try:
			...
			self._refresh_actions()
			return
		self.statusBar().showMessage(self.tr("Placed one Ferrum user template."), 5000)
		self._refresh_actions()
	...
	def _publish_native_user_template_capture(
		...
		if candidate.suffix != ".cdml":
			self._show_edit_refusal(self._unavailable_edit_refusal("Ferrum user templates use the lowercase .cdml extension."))
			return False
		if candidate.parent != directory:
			self._show_edit_refusal(self._unavailable_edit_refusal("Save templates directly in the configured Ferrum template directory."))
			return False
		if not self._native_user_template_save_is_current(capture):
			self._show_edit_refusal(self._unavailable_edit_refusal("The active Rust document changed; choose Save As User Template again."))
			return False
		try:
		...
		if (
			...
			self._show_edit_refusal(self._unavailable_edit_refusal("Inspect the destination before relying on this template."))
			return False
		self._user_template_catalog = self._scan_native_user_template_catalog()

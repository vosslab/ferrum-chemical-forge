...
def protocol_envelope(
		ferrum: pathlib.Path, operation: dict[str, object],
		) -> dict[str, object]:
	...
	result = run(ferrum, "protocol", "run", "-", input_text=request_text)
...
#============================================
def check_inspect(
		ferrum: pathlib.Path, source: pathlib.Path,
		) -> None:
	"""Compare inspect's human and envelope surfaces to protocol execution."""
	operation = {"kind": "document.inspect", "document": CDML}
	expected = protocol_envelope(ferrum, operation)
	json_result = run(ferrum, "inspect", str(source), "--json")
	...
	human_result = run(ferrum, "inspect", "-", input_text=CDML)
...
def check_validate(
	...
	expected = protocol_envelope(ferrum, operation)
	json_result = run(
		ferrum, "validate", str(source), "--level", "typed", "--json",
	)
	...
	human_result = run(
		ferrum, "validate", "-", "--level", "typed", input_text=CDML,
	)
	...
	refusal = run(ferrum, "validate", "-", input_text="not CDML")
...
def check_rewrite(
	...
	expected = protocol_envelope(ferrum, operation)
	json_result = run(ferrum, "rewrite", str(source), "--json")
	...
	file_result = run(
		ferrum, "rewrite", str(source), "--output", str(destination),
	)
...
def check_render(
	...
	expected = protocol_envelope(ferrum, operation)
	json_result = run(
		ferrum, "render", str(source), "--to", "svg", "--json",
	)
	...
	file_result = run(
		ferrum, "render", str(source), "--output", str(destination),
	)
...
def check_engine_verbs(
	...
	"""Prove the staged local runtime drives convert and coords."""
	...
	convert_expected = protocol_envelope(ferrum, convert_operation)
	convert_json = run(
		ferrum, "convert", str(convert_source), "--to", "smiles", "--json",
	)
	...
	coords_expected = protocol_envelope(ferrum, coords_operation)
	coords_json = run(ferrum, "coords", "-", "--json", input_text=CDML)
	...
	if "outcome" not in convert_expected or "outcome" not in coords_expected:
		raise VerbE2eError(
			"the executable-relative local runtime did not complete both engine operations: "
...
def check_exit_channels(
		ferrum: pathlib.Path, rewritten: str,
		) -> None:
	...
	missing_format = run(ferrum, "render", "-", input_text=CDML)
	require_exit(missing_format, 1, "render missing format")
	usage = run(ferrum, "validate")
...
def main() -> int:
	...
	runtime = ferrum.parent.parent / "runtime" / "engine-v1"
	if not runtime.is_dir():
		raise VerbE2eError(
			"--ferrum must name a staged local executable with build/runtime/engine-v1"
		)
	with tempfile.TemporaryDirectory(prefix="ferrum-verb-e2e-") as temp_text:
		...
		check_inspect(ferrum, source)
		check_validate(ferrum, source)

"""Application-owned preferences for the ordinary Ferrum window."""
...
#============================================
class FerrumNativePreferencesDialog(FerrumAccessibleDialog):
	...
	def __init__(self, current: FerrumNativePreferencesV1,
		...
		self.setTabOrder(self._theme, self._show_hex_grid)
		self.setTabOrder(
	...
	def choose_preferences(parent: PySide6.QtWidgets.QWidget,
		...
		"""Adapt the retained public wrapper to the standard preferences dialog."""
		chosen = ferrum_qt.dialogs.preferences_dialog.PreferencesDialog.choose_preferences(
			ferrum_qt.dialogs.preferences_dialog.PreferencesDialogResult(
				theme=current.theme,
				remember_workspace=current.remember_workspace,
				show_hex_grid=current.show_hex_grid,
				snap_authored_points_to_hex_grid=(
					current.snap_authored_points_to_hex_grid
				),
			), parent,
		...
		if chosen is None:
			return None
		return FerrumNativePreferencesV1(
			chosen.theme, chosen.remember_workspace, chosen.show_hex_grid,

...
def test_bond_properties_are_one_frozen_atomic_edit_with_history(
    ...
    refused = session.snapshot()
    assert (refused.revision, refused.digest) == (before.revision, before.digest)

    changes = (
        change_type.presentation(ferrum_chem.DocumentBondPresentationV1.normal_double),
        change_type.center(True),
        change_type.line_width(2.5),
        change_type.bond_width(4.0),
        change_type.wedge_width(None),
    ...
    assert change_type.__module__ == "ferrum_chem"
    assert not hasattr(ferrum_chem, "DocumentBondStyleV1")
    assert not hasattr(change_type, "order") and not hasattr(change_type, "style")
    assert changed.snapshot.revision == 1
    assert (
        bond.source_type, bond.presentation, bond.center, bond.line_width,
        bond.bond_width, bond.wedge_width, bond.color,
    ) == (
        "n2", ferrum_chem.DocumentBondPresentationV1.normal_double,
        True, 2.5, 4.0, None, "#aabbcc",
    )

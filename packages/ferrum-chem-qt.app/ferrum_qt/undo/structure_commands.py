...
class AddAtomCommand(PySide6.QtGui.QUndoCommand):
	...
	def graphics_items(self): return [self._atom_item]
...
class RemoveAtomCommand(PySide6.QtGui.QUndoCommand):
	...
	def __init__(self, scene, molecule_model, atom_model, atom_item, connected_bonds, text="Remove Atom"):
		super().__init__(text); self._scene = scene; self._molecule_model = molecule_model
	...
	def graphics_items(self): return [self._atom_item, *(item for _model, item in self._connected_bonds)]
...
class AddBondCommand(PySide6.QtGui.QUndoCommand):
	...
	def graphics_items(self): return [self._bond_item]
...
class MoveAtomsCommand(PySide6.QtGui.QUndoCommand):
	...
	_MERGE_ID = 1001
	def __init__(self, items_and_offsets, text="Move Atoms"):
	...
	def _apply_fragment_snapshots(self, after):
	...
	def graphics_items(self): return [item for item, _dx, _dy in self._items_and_offsets]

...
class FerrumNativePresentationStackMixin:
	...
	def reorder_selected_presentation_roots(self, order: object) -> object:
		...
		import ferrum_chem
		if type(order) is not ferrum_chem.DocumentPresentationStackOrderV1:
			raise TypeError("presentation stack order must be an exact frozen Rust value")
		minimum = (
			2
			if order == ferrum_chem.DocumentPresentationStackOrderV1.reverse_selected_slots
			else 1
		)
...
def _on_reorder_presentation(window: object, mode_name: str) -> None:
	...
	try:
		import ferrum_chem
		order = getattr(ferrum_chem.DocumentPresentationStackOrderV1, mode_name)
		tab.reorder_selected_presentation_roots(order)
	except Exception as exc:
		window._refresh_actions()
		window._show_native_file_warning("Native Presentation Stack Error", str(exc))
		return
	window.statusBar().showMessage(
		window.tr("Reordered Rust-native presentation."), 5000,
	)
	window._refresh_actions()
